CREATE OR REPLACE PACKAGE BODY "SBREXT_SET_ROW"
AS
   /*
   ...
   */
   ...
   IS
      --
      CURSOR c_VM (p_short_meaning VARCHAR2)
      IS
         SELECT vm_idseq
   ...
   ******************************************************************************/
   ...
   BEGIN
      ...
      IF P_ACTION NOT IN ('INS', 'UPD', 'DEL')
      THEN
      ...
      IF P_ACTION = 'INS'
      THEN                                         --we are inserting a record
         IF P_VD_VD_IDSEQ IS NOT NULL
         THEN
         ...
         ELSE
            ...
            IF P_VD_VERSION IS NULL
            THEN
               P_VD_VERSION :=
                    Sbrext_Common_Routines.get_ac_version (
                       p_vd_preferred_name,
                       p_vd_conte_idseq,
                       'VALUEDOMAIN',
                       'HIGHEST')
            ...
            IF P_VD_CD_IDSEQ IS NULL
            THEN
               P_VD_CD_IDSEQ :=
                  Sbrext_Common_Routines.get_cd (P_VD_CONTE_IDSEQ);
            ...
            IF P_VD_LATEST_VERSION_IND IS NULL
            THEN
               P_VD_LATEST_VERSION_IND := 'No';
            END IF;

            IF P_VD_TYPE_FLAG IS NULL
            THEN
               P_VD_TYPE_FLAG := 'N';
            ELSIF P_VD_TYPE_FLAG NOT IN ('E', 'N', 'R')
            THEN                         -- 26-Mar-2004, W. Ver Hoef - added R
               P_RETURN_CODE := 'API_VD_106'; --VD type flag can be ;E or N or OR only
               RETURN;
            END IF;

            IF P_VD_DTL_NAME IS NULL
            THEN
         ...
         IF UPPER (P_VD_PREFERRED_NAME) <> 'SYSTEM GENERATED'
         THEN
            SELECT COUNT (*)
              INTO v_count
              FROM value_Domains
      ...
      IF P_ACTION = 'UPD'
      THEN                                          --we are updating a record
         IF P_VD_VD_IDSEQ IS NULL
         THEN
         ...
         IF     (   P_VD_PREFERRED_NAME IS NOT NULL
                 OR P_VD_CONTE_IDSEQ IS NOT NULL)
            AND v_asl_name = 'RELEASED'
         THEN
            P_RETURN_CODE := 'API_VD_401'; --Preferred Name or Context Can not be updated
            RETURN;
         END IF;

         IF NOT Sbrext_Common_Routines.ac_exists (P_VD_VD_IDSEQ)
         THEN
         ...
         IF P_VD_TYPE_FLAG NOT IN ('E', 'N', 'R')
         THEN                            -- 26-Mar-2004, W. Ver Hoef - added R
            P_RETURN_CODE := 'API_VD_106'; --VD type flag can be ;E or N or OR only
            RETURN;
         END IF;
      ...
      IF P_ACTION = 'DEL'
      THEN                               --we are logically deleteing a record
         IF P_VD_VD_IDSEQ IS NULL
         THEN
      ...
      IF (P_VD_LATEST_VERSION_IND IS NOT NULL)
      THEN
         IF P_VD_LATEST_VERSION_IND NOT IN ('Yes', 'No')
         THEN
      ...
      IF NOT Sbrext_Common_Routines.valid_alphanumeric (P_VD_PREFERRED_NAME)
      THEN
      ...
      END IF;
      */
      IF NOT Sbrext_Common_Routines.valid_char (P_VD_PREFERRED_DEFINITION)
      THEN
         P_RETURN_CODE := 'API_VD_133'; -- Value Domain Preferred Definition has invalid characters
         RETURN;
      END IF;

      IF NOT Sbrext_Common_Routines.valid_char (P_VD_LONG_NAME)
      THEN
         P_RETURN_CODE := 'API_VD_134'; -- Value Domain Long Name has invalid characters
         RETURN;
      END IF;

      IF P_FORML_DESCRIPTION IS NOT NULL
      THEN
         IF NOT Sbrext_Common_Routines.valid_char (P_FORML_DESCRIPTION)
         THEN
      ...
      IF P_FORML_COMMENT IS NOT NULL
      THEN
         IF NOT Sbrext_Common_Routines.valid_char (P_FORML_COMMENT)
         THEN
      ...
      IF P_UOML_DESCRIPTION IS NOT NULL
      THEN
         IF NOT Sbrext_Common_Routines.valid_char (P_UOML_DESCRIPTION)
         THEN
      ...
      IF P_UOML_COMMENT IS NOT NULL
      THEN
         IF NOT Sbrext_Common_Routines.valid_char (P_UOML_COMMENT)
         THEN
      ...
      THEN
         ...
         IF NOT Sbrext_Common_Routines.context_exists (P_VD_CONTE_IDSEQ)
         THEN
      ...
      IF P_VD_CD_IDSEQ IS NOT NULL
      THEN                                         -- 24-Jul-2003, W. Ver Hoef
         IF NOT Sbrext_Common_Routines.ac_exists (P_VD_CD_IDSEQ)
         THEN
      ...
      IF P_VD_ASL_NAME IS NOT NULL
      THEN                                         -- 24-Jul-2003, W. Ver Hoef
         IF NOT Sbrext_Common_Routines.workflow_exists (P_VD_ASL_NAME)
         THEN
      ...
      IF (P_VD_CHAR_SET_NAME IS NOT NULL AND P_VD_CHAR_SET_NAME != ' ')
      THEN
         IF NOT Sbrext_Common_Routines.char_set_exists (P_VD_CHAR_SET_NAME)
         THEN
      ...
      IF P_VD_DTL_NAME IS NOT NULL
      THEN                                         -- 24-Jul-2003, W. Ver Hoef
         IF NOT Sbrext_Common_Routines.data_type_exists (P_VD_DTL_NAME)
         THEN
      ...
      IF P_VD_REP_IDSEQ IS NOT NULL AND P_VD_REP_IDSEQ != ' '
      THEN
         IF NOT Sbrext_Common_Routines.ac_exists (P_VD_REP_IDSEQ)
         THEN
      ...
      IF P_VD_QUALIFIER_NAME IS NOT NULL AND P_VD_QUALIFIER_NAME != ' '
      THEN
         IF NOT Sbrext_Common_Routines.qual_exists (P_VD_QUALIFIER_NAME)
         THEN
      ...
      IF (P_VD_BEGIN_DATE IS NOT NULL)
      THEN
         Sbrext_Common_Routines.valid_date (P_RETURN_CODE,
                                            P_VD_BEGIN_DATE,
                                            v_begin_date);
      ...
      IF (P_VD_BEGIN_DATE IS NOT NULL AND P_VD_END_DATE IS NOT NULL)
      THEN
         IF (v_end_date < v_begin_date)
         THEN
      ...
      IF (    P_VD_FORML_NAME IS NULL
          AND (P_FORML_DESCRIPTION IS NOT NULL OR P_FORML_COMMENT IS NOT NULL))
      THEN
         P_RETURN_CODE := 'API_VD_205'; --cannot send in format description and comments without format name
         RETURN;
      END IF;

      IF (    P_VD_UOML_NAME IS NULL
          AND (P_UOML_DESCRIPTION IS NOT NULL OR P_UOML_COMMENT IS NOT NULL))
      THEN
      ...
      IF (P_VD_FORML_NAME IS NOT NULL AND P_VD_FORML_NAME != ' ')
      THEN
         IF NOT (Sbrext_Common_Routines.vd_format_exists (P_VD_FORML_NAME))
         THEN
      ...
      IF (P_VD_UOML_NAME IS NOT NULL AND P_VD_UOML_NAME != ' ')
      THEN
         IF NOT (Sbrext_Common_Routines.uoml_exists (P_VD_UOML_NAME))
         THEN
      ...
      THEN
         ...
         IF Sbrext_Common_Routines.ac_exists (P_VD_PREFERRED_NAME,
                                              P_VD_CONTE_IDSEQ,
                                              P_VD_VERSION,
                                              'VALUEDOMAIN')
         THEN
         ...
         IF Sbrext_Common_Routines.ac_version_exists (P_VD_PREFERRED_NAME,
                                                      P_VD_CONTE_IDSEQ,
                                                      'VALUEDOMAIN')
         THEN                                 -- we are creating a new version
            v_new_version := TRUE;
            v_ac :=
               Sbrext_Common_Routines.get_version_ac (P_VD_PREFERRED_NAME,
                                                      P_VD_CONTE_IDSEQ,
                                                      'VALUEDOMAIN');
         END IF;

         IF p_vd_preferred_name = 'System Generated'
         THEN
         ...
         BEGIN
            ...
            THEN
               p_vd_preferred_name :=
                  sbrext_common_routines.GENERATE_VD_PREFERRED_NAME (
                     p_vd_vd_idseq);
         ...
         IF (P_VD_LATEST_VERSION_IND = 'Yes')
         THEN
            Sbrext_Common_Routines.set_ac_lvi (P_RETURN_CODE,
                                               P_VD_VD_IDSEQ,
                                               'VALUEDOMAIN');
         ...
         THEN
            BEGIN
               meta_config_mgmt.CREATE_AC_HISTORIES (v_ac,
                                                     P_VD_VD_IDSEQ,
                                                     'VERSIONED',
                                                     'VALUEDOMAIN');
            EXCEPTION
      ...
      THEN
         ...
         IF P_VD_VERSION IS NOT NULL
         THEN                                      -- 24-JUL-2003, W. Ver Hoef
            IF v_version <> P_VD_VERSION
            THEN
         ...
         IF P_VD_LONG_NAME IS NULL
         THEN
            v_vd_ind.long_name := FALSE;
         ELSIF P_VD_LONG_NAME = ' '
         THEN -- Condition added on 9/9/03 to allow null updates by Prerna Aggarwal
         ...
         IF P_VD_LOW_VALUE_NUM IS NULL
         THEN
            v_vd_ind.low_value_num := FALSE;
         ELSIF P_VD_low_value_num = ' '
         THEN -- Condition added on 9/9/03 to allow null updates by Prerna Aggarwal
         ...
         IF P_VD_HIGH_VALUE_NUM IS NULL
         THEN
            v_vd_ind.high_Value_num := FALSE;
         ELSIF P_VD_high_value_num = ' '
         THEN -- Condition added on 9/9/03 to allow null updates by Prerna Aggarwal
         ...
         IF P_VD_MIN_LENGTH_NUM IS NULL
         THEN
            v_vd_ind.min_length_num := FALSE;
         ELSIF P_VD_min_length_num = -1
         THEN -- Condition added on 9/9/03 to allow null updates by Prerna Aggarwal
         ...
         IF P_VD_MAX_LENGTH_NUM IS NULL
         THEN
            v_vd_ind.max_length_num := FALSE;
         ELSIF P_VD_max_length_num = -1
         THEN -- Condition added on 9/9/03 to allow null updates by Prerna Aggarwal
         ...
         IF P_VD_DECIMAL_PLACE IS NULL
         THEN
            v_vd_ind.decimal_place := FALSE;
         ELSIF P_VD_decimal_place = -1
         THEN -- Condition added on 9/9/03 to allow null updates by Prerna Aggarwal
         ...
         IF P_VD_CHAR_SET_NAME IS NULL
         THEN
            v_vd_ind.char_set_name := FALSE;
         ELSIF P_VD_char_set_name = ' '
         THEN -- Condition added on 9/9/03 to allow null updates by Prerna Aggarwal
         ...
         IF P_VD_BEGIN_DATE IS NULL
         THEN
            v_vd_ind.begin_date := FALSE;
         ELSIF P_VD_begin_date = ' '
         THEN -- Condition added on 9/9/03 to allow null updates by Prerna Aggarwal
         ...
         IF P_VD_CHANGE_NOTE IS NULL
         THEN
            v_vd_ind.change_note := FALSE;
         ELSIF P_VD_change_note = ' '
         THEN -- Condition added on 9/9/03 to allow null updates by Prerna Aggarwal
         ...
         IF P_VD_REP_IDSEQ IS NULL
         THEN
            v_vd_ind.rep_idseq := FALSE;
         ELSIF P_VD_REP_IDSEQ = ' '
         THEN -- Condition added on 7/30/03 to allow null updates by Prerna Aggarwal
         ...
         IF P_VD_QUALIFIER_NAME IS NULL
         THEN
            v_vd_ind.qualifier_name := FALSE;
         ELSIF P_VD_QUALIFIER_NAME = ' '
         THEN -- Condition added on 7/30/03 to allow null updates by Prerna Aggarwal
   ...
   BEGIN
      ...
      IF P_VD_CON_ARRAY IS NOT NULL
      THEN
         P_VD_CONDR_IDSEQ :=
            sbrext_common_routines.SET_DERIVATION_RULE (P_VD_CON_ARRAY);

         IF P_VD_CONDR_IDSEQ = 'Concept Not Found'
         THEN
      ...
      IF P_VD_CONDR_IDSEQ IS NOT NULL
      THEN
         IF NOT sbrext_common_routines.CONDR_EXISTS (P_VD_CONDR_IDSEQ)
         THEN
      ...
      */
      ...
      THEN
         ...
         IF P_VD_CONDR_IDSEQ IS NULL
         THEN
            v_vd_ind.condr_idseq := FALSE;
         ELSIF P_VD_CONDR_IDSEQ = ' '
         THEN
   ...
   BEGIN
      ...
      THEN                                         --we are inserting a record
         ...
         ELSE
            ...
            IF P_DEC_CD_IDSEQ IS NULL
            THEN
               P_DEC_CD_IDSEQ :=
                  Sbrext_Common_Routines.get_cd (P_DEC_CONTE_IDSEQ);
            ...
            IF P_DEC_LATEST_VERSION_IND IS NULL
            THEN
      ...
      THEN                                          --we are updating a record
         ...
         IF     (   P_DEC_PREFERRED_NAME IS NOT NULL
                 OR P_DEC_CONTE_IDSEQ IS NOT NULL)
            AND v_asl_name = 'RELEASED'
         THEN
         ...
         IF NOT Sbrext_Common_Routines.ac_exists (P_DEC_DEC_IDSEQ)
         THEN
      ...
      IF P_DEC_LATEST_VERSION_IND IS NOT NULL
      THEN
         IF P_DEC_LATEST_VERSION_IND NOT IN ('Yes', 'No')
         THEN
      ...
      --Check to see that all VARCHAR2 and  VARCHAR2 parameters have correct length
      IF LENGTH (P_DEC_PREFERRED_NAME) >
            Sbrext_Column_Lengths.L_DEC_PREFERRED_NAME
      THEN
         P_RETURN_CODE := 'API_DEC_111'; --Length of preferred_name exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_DEC_PREFERRED_DEFINITION) >
            Sbrext_Column_Lengths.L_DEC_PREFERRED_DEFINITION
      THEN
         P_RETURN_CODE := 'API_DEC_113'; --Length of Preferred_definition exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_DEC_LONG_NAME) > Sbrext_Column_Lengths.L_DEC_LONG_NAME
      THEN
         P_RETURN_CODE := 'API_DEC_114'; --Length of Long_name exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_DEC_ASL_NAME) > Sbrext_Column_Lengths.L_DEC_ASL_NAME
      THEN
      ...
      IF LENGTH(P_DEC_PROPL_NAME) > Sbrext_Column_Lengths.L_DEC_PROPL_NAME THEN
        ...
        RETURN;
      END IF;*/
      IF LENGTH (P_DEC_PROPERTY_QUALIFIER) >
            Sbrext_Column_Lengths.L_DEC_PROPERTY_QUALIFIER
      THEN
         P_RETURN_CODE := 'API_DEC_120'; --Length of Property qualifier exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_DEC_OBJ_CLASS_QUALIFIER) >
            Sbrext_Column_Lengths.L_DEC_OBJ_CLASS_QUALIFIER
      THEN
         P_RETURN_CODE := 'API_DEC_121'; --Length of Object Class qualifier exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_DEC_CHANGE_NOTE) > Sbrext_Column_Lengths.L_DEC_CHANGE_NOTE
      THEN
      ...
      --check to see that charachter strings are valid
      IF     NOT Sbrext_Common_Routines.valid_alphanumeric (
                    P_DEC_PREFERRED_NAME)
         AND P_DEC_PREFERRED_NAME IS NOT NULL
      THEN
         P_RETURN_CODE := 'API_DEC_130'; -- Data Element Concepts Preferred Name has invalid Ccharacters
         RETURN;
      END IF;

      IF     NOT Sbrext_Common_Routines.valid_char (
                    P_DEC_PREFERRED_DEFINITION)
         AND P_DEC_PREFERRED_DEFINITION IS NOT NULL
      THEN
      ...
      IF     NOT Sbrext_Common_Routines.valid_char (P_DEC_LONG_NAME)
         AND P_DEC_LONG_NAME IS NOT NULL
      THEN
      ...
      IF NOT Sbrext_Common_Routines.context_exists (P_DEC_CONTE_IDSEQ)
      THEN
         IF P_DEC_CONTE_IDSEQ IS NOT NULL
         THEN
      ...
      IF NOT Sbrext_Common_Routines.ac_exists (P_DEC_CD_IDSEQ)
      THEN
         IF P_DEC_CD_IDSEQ IS NOT NULL
         THEN
      ...
      IF NOT Sbrext_Common_Routines.workflow_exists (P_DEC_ASL_NAME)
      THEN
         IF P_DEC_ASL_NAME IS NOT NULL
         THEN
      ...
      IF P_DEC_PROP_IDSEQ IS NOT NULL AND P_DEC_PROP_IDSEQ != ' '
      THEN
         IF NOT Sbrext_Common_Routines.ac_exists (P_DEC_PROP_IDSEQ)
         THEN
      ...
      IF P_DEC_OC_IDSEQ IS NOT NULL AND P_DEC_OC_IDSEQ != ' '
      THEN
         IF NOT Sbrext_Common_Routines.ac_exists (P_DEC_OC_IDSEQ)
         THEN
      ...
      IF     P_DEC_PROPERTY_QUALIFIER IS NOT NULL
         AND P_DEC_PROPERTY_QUALIFIER != ' '
      THEN
         IF NOT Sbrext_Common_Routines.qual_exists (P_DEC_PROPERTY_QUALIFIER)
         THEN
      ...
      IF     P_DEC_OBJ_CLASS_QUALIFIER IS NOT NULL
         AND P_DEC_OBJ_CLASS_QUALIFIER != ' '
      THEN
         IF NOT Sbrext_Common_Routines.qual_exists (
      ...
      --check to see that begin data and end date are valid dates
      IF (P_DEC_BEGIN_DATE IS NOT NULL)
      THEN
         Sbrext_Common_Routines.valid_date (P_RETURN_CODE,
                                            P_DEC_BEGIN_DATE,
                                            v_begin_date);
      ...
      IF (P_DEC_BEGIN_DATE IS NOT NULL AND P_DEC_END_DATE IS NOT NULL)
      THEN
         IF (v_end_date < v_begin_date)
         THEN
      ...
      THEN
         ...
         --Preferred Name, Context and Version does not already exist
         IF Sbrext_Common_Routines.ac_exists (P_DEC_PREFERRED_NAME,
                                              P_DEC_CONTE_IDSEQ,
                                              P_DEC_VERSION,
                                              'DE_CONCEPT')
         THEN
         ...
         --Check to see if prior versions alresdy exist
         IF Sbrext_Common_Routines.ac_version_exists (P_DEC_PREFERRED_NAME,
                                                      P_DEC_CONTE_IDSEQ,
                                                      'DE_CONCEPT')
         THEN                                 -- we are creating a new version
            v_new_version := TRUE;
            v_ac :=
               Sbrext_Common_Routines.get_version_ac (P_DEC_PREFERRED_NAME,
                                                      P_DEC_CONTE_IDSEQ,
                                                      'DE_CONCEPT');
         END IF;
         ...
         --If LATEST_VERSION_IND is'Yes' then update so that all other versions have the indicator set to 'No'
         IF (P_DEC_LATEST_VERSION_IND = 'Yes')
         THEN
            Sbrext_Common_Routines.set_ac_lvi (P_RETURN_CODE,
                                               P_DEC_DEC_IDSEQ,
                                               'DE_CONCEPT');
      ...
      THEN
         ...
         IF P_DEC_LONG_NAME IS NULL
         THEN
            v_dec_ind.long_name := FALSE;
         ELSIF P_dec_long_name = ' '
         THEN -- Condition added on 9/9/03 to allow null updates by Prerna Aggarwal
         ...
         /* IF P_DEC_PROPL_NAME IS NULL THEN
          ...
          END IF;
          */
         IF P_DEC_PROPERTY_QUALIFIER IS NULL
         THEN
            v_dec_ind.property_qualifier := FALSE;
         ELSIF P_DEC_PROPERTY_QUALIFIER = ' '
         THEN -- Condition added on 7/30/03 to allow null updates by Prerna Aggarwal
         ...
         IF P_DEC_OBJ_CLASS_QUALIFIER IS NULL
         THEN
            v_dec_ind.obj_class_qualifier := FALSE;
         ELSIF P_DEC_OBJ_CLASS_QUALIFIER = ' '
         THEN -- Condition added on 7/30/03 to allow null updates by Prerna Aggarwal
         ...
         IF P_DEC_BEGIN_DATE IS NULL
         THEN
            v_dec_ind.begin_date := FALSE;
         ELSIF P_DEC_begin_date = ' '
         THEN -- Condition added on 9/9/03 to allow null updates by Prerna Aggarwal
         ...
         IF P_DEC_END_DATE IS NULL
         THEN
            v_dec_ind.end_date := FALSE;
         ELSIF P_DEC_end_date = ' '
         THEN -- Condition added on 9/9/03 to allow null updates by Prerna Aggarwal
         ...
         IF P_DEC_CHANGE_NOTE IS NULL
         THEN
            v_dec_ind.change_note := FALSE;
         ELSIF P_DEC_change_note = ' '
         THEN -- Condition added on 9/9/03 to allow null updates by Prerna Aggarwal
         ...
         IF P_DEC_OC_IDSEQ IS NULL
         THEN
            v_dec_ind.oc_idseq := FALSE;
         ELSIF P_DEC_OC_IDSEQ = ' '
         THEN -- Condition added on 7/30/03 to allow null updates by Prerna Aggarwal
         ...
         IF P_DEC_PROP_IDSEQ IS NULL
         THEN
            v_dec_ind.prop_idseq := FALSE;
         ELSIF P_DEC_PROP_IDSEQ = ' '
         THEN -- Condition added on 7/30/03 to allow null updates by Prerna Aggarwal
         ...
         IF P_DEC_ORIGIN IS NULL
         THEN                                      -- 15-Jul-2003, W. Ver Hoef
            v_dec_ind.origin := FALSE;
         ELSIF P_DEC_origin = ' '
         THEN -- Condition added on 9/9/03 to allow null updates by Prerna Aggarwal
   ...
   PROCEDURE SET_VM (P_RETURN_CODE           OUT VARCHAR2,
                     P_ACTION             IN     VARCHAR2,
   ...
   BEGIN
      ...
      THEN
         ...
         IF NOT Sbrext_Common_Routines.vm_exists (
      ...
      --Check to see that all VARCHAR2 and  VARCHAR2 parameters have correct length
      IF LENGTH (P_VM_SHORT_MEANING) >
            Sbrext_Column_Lengths.L_VM_SHORT_MEANING
      THEN
         P_RETURN_CODE := 'API_VM_111'; --Length of SHORT_MEANING exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_VM_DESCRIPTION) > Sbrext_Column_Lengths.L_VM_DESCRIPTION
      THEN
         P_RETURN_CODE := 'API_VM_113'; --Length of DESCRIPTION exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_VM_COMMENTS) > Sbrext_Column_Lengths.L_VM_COMMENTS
      THEN
      ...
      --check to see that charachter strings are valid
      IF NOT Sbrext_Common_Routines.valid_char (P_VM_SHORT_MEANING)
      THEN
         P_RETURN_CODE := 'API_VM_130'; -- Value Meaning Short Meaning has invalid characters
         RETURN;
      END IF;

      IF NOT Sbrext_Common_Routines.valid_char (P_VM_DESCRIPTION)
      THEN
         P_RETURN_CODE := 'API_VM_133'; -- Value Meaning Description has invalid characters
         RETURN;
      END IF;

      IF NOT Sbrext_Common_Routines.valid_char (P_VM_COMMENTS)
      THEN
      ...
      IF (P_VM_BEGIN_DATE IS NOT NULL)
      THEN
         Sbrext_Common_Routines.valid_date (P_RETURN_CODE,
                                            P_VM_BEGIN_DATE,
                                            V_BEGIN_DATE);
      ...
      IF (P_VM_BEGIN_DATE IS NOT NULL AND P_VM_END_DATE IS NOT NULL)
      THEN
         IF (v_end_date < v_begin_date)
         THEN
            ...
            RETURN;
         END IF;
      ELSIF (P_VM_END_DATE IS NOT NULL AND P_VM_BEGIN_DATE IS NULL)
      THEN
      ...
      THEN
         ...
         IF Sbrext_Common_Routines.vm_exists (
               p_short_meaning   => P_VM_SHORT_MEANING)
         THEN
   ...
   */
   ...
   PROCEDURE SET_VM (P_UA_NAME                IN     VARCHAR2,
                     P_RETURN_CODE               OUT VARCHAR2,
   ...
   BEGIN
      ...
      IF P_CON_ARRAY IS NOT NULL
      THEN
         SBREXT.SBREXT_GET_ROW.GET_VM_CONDR (P_CON_ARRAY,
                                             P_RETURN_CODE,
                                             P_LONG_NAME,
                                             P_CONDR_IDSEQ,
      ...
      THEN
         ...
         IF P_VM_IDSEQ IS NULL
         THEN
            P_RETURN_CODE := 'API_VM_102'; --Value Meaning idseq cannot be null here
            RETURN;
         END IF;

         IF NOT Sbrext_Common_Routines.vm_exists (p_vm_idseq => P_VM_IDSEQ)
         THEN
            P_RETURN_CODE := 'API_VM_005';                      --VM not found
            RETURN;
         END IF;
      ...
      --Check to see that all VARCHAR2 and  VARCHAR2 parameters have correct length
      IF LENGTH (P_LONG_NAME) > Sbrext_Column_Lengths.L_VM_LONG_NAME
      THEN
         P_RETURN_CODE := 'API_VM_111'; --Length of SHORT_MEANING exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_PREFERRED_DEFINITION) >
            Sbrext_Column_Lengths.L_VM_PREFERRED_DEFINITION
      THEN
         P_RETURN_CODE := 'API_VM_113'; --Length of DESCRIPTION exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_CHANGE_NOTE) > Sbrext_Column_Lengths.L_VM_CHANGE_NOTE
      THEN
         P_RETURN_CODE := 'API_VM_114'; --Length of COMMENTS exceeds maximum length
         RETURN;
      END IF;
      ...
      --check to see that charachter strings are valid
      IF NOT Sbrext_Common_Routines.valid_char (P_LONG_NAME)
      THEN
         P_RETURN_CODE := 'API_VM_130'; -- Value Meaning Short Meaning has invalid characters
         RETURN;
      END IF;

      IF NOT Sbrext_Common_Routines.valid_char (P_PREFERRED_DEFINITION)
      THEN
         P_RETURN_CODE := 'API_VM_133'; -- Value Meaning Description has invalid characters
         RETURN;
      END IF;

      IF NOT Sbrext_Common_Routines.valid_char (P_CHANGE_NOTE)
      THEN
         P_RETURN_CODE := 'API_VM_134'; -- Value Meaning Comment has invalid characters
         RETURN;
      END IF;
      ...
      IF (P_BEGIN_DATE IS NOT NULL AND P_END_DATE IS NOT NULL)
      THEN
         IF (v_end_date < v_begin_date)
         THEN
            P_RETURN_CODE := 'API_VM_210';     --end date is before begin date
      ...
      THEN
         ...
         IF P_PREFERRED_DEFINITION IS NULL
         THEN
         ...
         IF P_LATEST_VERSION_IND IS NULL
         THEN
      ...
      --return all the data
      sbrext.sbrext_get_row.get_vm (P_RETURN_CODE,
                                    P_VM_IDSEQ,
                                    P_LONG_NAME,
                                    P_VERSION,
                                    P_VM_ID,
   ...
   PROCEDURE SET_VM_CONDR (P_vm_con_array       IN     VARCHAR2,
                           P_RETURN_CODE           OUT VARCHAR2,
                           P_VM_SHORT_MEANING   IN OUT VARCHAR2,
   ...
   BEGIN
      ...
      v_condr_idseq :=
         sbrext_common_routines.SET_DERIVATION_RULE (p_vm_con_array);

      IF NOT Sbrext_Common_Routines.CONDR_EXISTS (v_condr_idseq)
      THEN
      ...
      EXCEPTION
         WHEN NO_DATA_FOUND
         THEN
            Sbrext_Common_Routines.set_vm_condr (
               p_condr_idseq     => v_condr_idseq,
               p_return_code     => p_return_code,
               p_short_meaning   => p_vm_short_meaning);
      END;

      IF p_return_code IS NULL
      THEN
         SELECT SHORT_MEANING,
                DESCRIPTION,
                COMMENTS,
   ...
   BEGIN
      ...
      sbrext_common_routines.set_oc_rep_prop (
         p_condr_idseq   => v_condr_idseq,
         p_return_code   => p_return_code,
         p_conte_idseq   => NULL,
   ...
   PROCEDURE SET_PV (P_RETURN_CODE                 OUT VARCHAR2,
                     P_ACTION                   IN     VARCHAR2,
   ...
   BEGIN
      ...
      IF P_ACTION = 'UPD'
      THEN                                         --we are inserting a record
         IF P_PV_PV_IDSEQ IS NULL
         THEN
            P_RETURN_CODE := 'API_PV_400';   --for updates the ID is mandatory
            RETURN;
         ELSE
            IF NOT Sbrext_Common_Routines.pv_exists (
                      p_pv_idseq   => P_PV_PV_IDSEQ)
            THEN
      ...
      IF LENGTH (P_PV_VALUE) > Sbrext_Column_Lengths.L_PV_VALUE
      THEN
         P_RETURN_CODE := 'API_PV_111'; --Length of Value exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_PV_SHORT_MEANING) >
            Sbrext_Column_Lengths.L_PV_SHORT_MEANING
      THEN
         P_RETURN_CODE := 'API_PV_113'; --Length of Short Meaning exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_PV_MEANING_DESCRIPTION) >
            Sbrext_Column_Lengths.L_PV_MEANING_DESCRIPTION
      THEN
      ...
      --check to see that charachter strings are valid
      IF NOT Sbrext_Common_Routines.valid_char (P_PV_VALUE)
      THEN
         P_RETURN_CODE := 'API_PV_130'; -- Permissible Value Value has invalid characters
         RETURN;
      END IF;

      IF NOT Sbrext_Common_Routines.valid_char (P_PV_SHORT_MEANING)
      THEN
         P_RETURN_CODE := 'API_PV_133'; -- Permissible Value Short Meaning has invalid characters
         RETURN;
      END IF;

      IF NOT Sbrext_Common_Routines.valid_char (P_PV_MEANING_DESCRIPTION)
      THEN
      ...
      IF NOT Sbrext_Common_Routines.vm_exists (
                p_short_meaning   => P_PV_SHORT_MEANING)
      THEN
      ...
      IF (P_PV_BEGIN_DATE IS NOT NULL)
      THEN
         Sbrext_Common_Routines.valid_date (P_RETURN_CODE,
                                            P_PV_BEGIN_DATE,
                                            v_begin_date);
      ...
      IF (P_PV_BEGIN_DATE IS NOT NULL AND P_PV_END_DATE IS NOT NULL)
      THEN
         IF (v_end_date < v_begin_date)
         THEN
      ...
      THEN
         ...
         IF Sbrext_Common_Routines.pv_exists (
               p_value           => P_PV_VALUE,
               p_short_meaning   => P_PV_SHORT_MEANING)
         THEN
      ...
      THEN
         ...
         IF P_PV_MEANING_DESCRIPTION IS NULL
         THEN
         ...
         IF P_PV_HIGH_VALUE_NUM IS NULL
         THEN
   ...
   BEGIN
      ...
      IF LENGTH (P_PV_VM_IDSEQ) > Sbrext_Column_Lengths.L_VM_IDSEQ
      THEN
         P_RETURN_CODE := 'API_PV_113'; --Length of Short Meaning exceeds maximum length
         RETURN;
      END IF;
      ...
      IF NOT Sbrext_Common_Routines.vm_exists (p_vm_idseq => P_PV_VM_IDSEQ)
      THEN
         P_RETURN_CODE := 'API_PV_200'; --Short Meaning notfound in the database
         RETURN;
      END IF;
      ...
      THEN
         --check to see that Value  does not already
         IF Sbrext_Common_Routines.pv_exists (p_value      => P_PV_VALUE,
                                              p_vm_idseq   => P_PV_VM_IDSEQ)
         THEN
            p_return_code := 'API_PV_300'; -- PERMISSIBLE VALUE ALREADY EXISTS
            RETURN;
         END IF;
   ...
   /*
    * *** DEPRECATED! *** This is not called directly from Java but from PL/SQL codes.
   */
   ...
   BEGIN
      DBMS_OUTPUT.put_line (
            'SET_VD_PVS (PL/SQL) entered P_VDPVS_PV_IDSEQ ['
         || P_VDPVS_PV_IDSEQ
         || '] P_VDPVS_CONTE_IDSEQ ['
         || P_VDPVS_CONTE_IDSEQ
         || ']');
      ...
      IF P_ACTION NOT IN ('INS', 'DEL', 'UPD')
      THEN
     ...
     RETURN;
      ...
      IF P_ACTION = 'DEL'
      THEN                                          --we are deleting a record
         IF P_VDPVS_VP_IDSEQ IS NULL
         THEN
            P_RETURN_CODE := 'API_VDPVS_400'; --for deleted the ID id mandatory
            RETURN;
         ELSE
            IF NOT Sbrext_Common_Routines.vd_pvs_exists (P_VDPVS_VP_IDSEQ)
            THEN
         ...
         IF Sbrext_Common_Routines.vd_pvs_qc_exists (P_VDPVS_VP_IDSEQ, NULL) =
               'TRUE'
         THEN
      ...
      IF NOT Sbrext_Common_Routines.ac_exists (P_VDPVS_VD_IDSEQ)
      THEN
         P_RETURN_CODE := 'API_VDPVS_201'; --VALUE Domain NOT found in the database
         RETURN;
      END IF;

      IF P_VDPVS_CON_IDSEQ IS NOT NULL
      THEN
         IF NOT Sbrext_Common_Routines.ac_exists (P_VDPVS_CON_IDSEQ)
         THEN
            P_RETURN_CODE := 'API_VDPVS_205'; --CONCEPT  NOT  found in the database
            RETURN;
         END IF;

         IF NOT Sbrext_Common_Routines.VALID_PARENT_CONCEPT (
                   p_vdpvs_con_idseq,
                   p_vdpvs_vd_idseq)
         THEN
      ...
      IF NOT Sbrext_Common_Routines.pv_exists (p_pv_idseq => P_VDPVS_PV_IDSEQ)
      THEN
         P_RETURN_CODE := 'API_VDPVS_202'; --PERMISSIBLE VALUE NOT found in the database
         RETURN;
      END IF;

      IF P_VDPVS_CONTE_IDSEQ IS NOT NULL
      THEN
         IF NOT Sbrext_Common_Routines.context_exists (P_VDPVS_CONTE_IDSEQ)
         THEN
      ...
      IF (P_VDPVS_BEGIN_DATE IS NOT NULL)
      THEN
         Sbrext_Common_Routines.valid_date (P_RETURN_CODE,
                                            P_VDPVS_BEGIN_DATE,
                                            v_begin_date);
      ...
      IF (P_VDPVS_END_DATE IS NOT NULL)
      THEN
         Sbrext_Common_Routines.valid_date (P_RETURN_CODE,
                                            P_VDPVS_END_DATE,
                                            v_end_date);
      ...
      IF (P_VDPVS_BEGIN_DATE IS NOT NULL AND P_VDPVS_END_DATE IS NOT NULL)
      THEN
         IF (v_end_date < v_begin_date)
         THEN
      ...
      THEN
         ...
         IF Sbrext_Common_Routines.vd_pvs_exists (P_VDPVS_VD_IDSEQ,
                                                  P_VDPVS_PV_IDSEQ)
         THEN
         ...
         END;
      ELSIF (P_ACTION = 'UPD')
      THEN                                   --  Added 8/23/2004 sprf_2.1.1_2b
         IF P_VDPVS_ORIGIN = ' '
         THEN
   ...
   BEGIN
      ...
      THEN                                          --we are updating a record
         ...
         IF     (   P_DE_PREFERRED_NAME IS NOT NULL
                 OR P_DE_CONTE_IDSEQ IS NOT NULL)
            AND v_asl_name = 'RELEASED'
         THEN
         ...
         IF NOT Sbrext_Common_Routines.ac_exists (P_DE_DE_IDSEQ)
         THEN
         ...
         DBMS_OUTPUT.put_line (
            NVL (admin_security_util.g_effective_user, 'NULL'));
      END IF;
      ...
      IF P_DE_LATEST_VERSION_IND IS NOT NULL
      THEN
         IF P_DE_LATEST_VERSION_IND NOT IN ('Yes', 'No')
         THEN
      ...
      --Check to see that all VARCHAR2 and  VARCHAR2 parameters have correct length
      IF LENGTH (P_DE_PREFERRED_NAME) >
            Sbrext_Column_Lengths.L_DE_PREFERRED_NAME
      THEN
         P_RETURN_CODE := 'API_DE_111'; --Length of preferred_name exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_DE_PREFERRED_DEFINITION) >
            Sbrext_Column_Lengths.L_DE_PREFERRED_DEFINITION
      THEN
         P_RETURN_CODE := 'API_DE_113'; --Length of Preferred_definition exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_DE_LONG_NAME) > Sbrext_Column_Lengths.L_DE_LONG_NAME
      THEN
         P_RETURN_CODE := 'API_DE_114'; --Length of Long_name exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_DE_ASL_NAME) > Sbrext_Column_Lengths.L_DE_ASL_NAME
      THEN
         P_RETURN_CODE := 'API_DE_115'; --Length of asl_name exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_DE_CHANGE_NOTE) > Sbrext_Column_Lengths.L_DE_CHANGE_NOTE
      THEN
      ...
      --check to see that charachter strings are valid
      IF NOT Sbrext_Common_Routines.valid_alphanumeric (P_DE_PREFERRED_NAME)
      THEN
         P_RETURN_CODE := 'API_DE_130'; -- Data Element Preferred Name has invalid Ccharacters
         RETURN;
      END IF;

      IF NOT Sbrext_Common_Routines.valid_char (P_DE_PREFERRED_DEFINITION)
      THEN
         P_RETURN_CODE := 'API_DE_133'; -- Data Element Preferred Definition has invalid characters
         RETURN;
      END IF;

      IF NOT Sbrext_Common_Routines.valid_char (P_DE_LONG_NAME)
      THEN
      ...
      IF P_DE_CONTE_IDSEQ IS NOT NULL
      THEN
         IF NOT Sbrext_Common_Routines.context_exists (P_DE_CONTE_IDSEQ)
         THEN
      ...
      IF P_DE_VD_IDSEQ IS NOT NULL
      THEN
         IF NOT Sbrext_Common_Routines.ac_exists (P_DE_VD_IDSEQ)
         THEN
      ...
      IF P_DE_ASL_NAME IS NOT NULL
      THEN
         IF NOT Sbrext_Common_Routines.workflow_exists (P_DE_ASL_NAME)
         THEN
      ...
      IF P_DE_DEC_IDSEQ IS NOT NULL
      THEN
         IF NOT Sbrext_Common_Routines.ac_exists (P_DE_DEC_IDSEQ)
         THEN
      ...
      IF (P_DE_BEGIN_DATE IS NOT NULL)
      THEN
         Sbrext_Common_Routines.valid_date (P_RETURN_CODE,
                                            P_DE_BEGIN_DATE,
                                            v_begin_date);
      ...
      IF (P_DE_BEGIN_DATE IS NOT NULL AND P_DE_END_DATE IS NOT NULL)
      THEN
         IF (v_end_date < v_begin_date)
         THEN
      ...
      THEN
         ...
         -- Preferred Name, Context and Version does not already exist
         IF Sbrext_Common_Routines.ac_exists (P_DE_PREFERRED_NAME,
                                              P_DE_CONTE_IDSEQ,
                                              P_DE_VERSION,
                                              'DATAELEMENT')
         THEN
         ...
         -- Check to see if prior versions alresdy exist
         IF Sbrext_Common_Routines.ac_version_exists (P_DE_PREFERRED_NAME,
                                                      P_DE_CONTE_IDSEQ,
                                                      'DATAELEMENT')
         THEN                                 -- we are creating a new version
            v_new_version := TRUE;
            v_ac :=
               Sbrext_Common_Routines.get_version_ac (P_DE_PREFERRED_NAME,
                                                      P_DE_CONTE_IDSEQ,
                                                      'DATAELEMENT');
         END IF;
         ...
         --If LATEST_VERSION_IND is 'Yes' then update so that all other versions have the indicator set to 'No'
         IF (P_DE_LATEST_VERSION_IND = 'Yes')
         THEN
            Sbrext_Common_Routines.set_ac_lvi (P_RETURN_CODE,
                                               P_DE_DE_IDSEQ,
                                               'DATAELEMENT');
      ...
      THEN
         ...
         IF P_DE_LONG_NAME IS NULL
         THEN
            v_de_ind.long_name := FALSE;
         ELSIF P_DE_long_name = ' '
         THEN -- Condition added on 9/9/03 to allow null updates by Prerna Aggarwal
         ...
         IF P_DE_BEGIN_DATE IS NULL
         THEN
            v_de_ind.begin_date := FALSE;
         ELSIF P_DE_begin_date = ' '
         THEN -- Condition added on 9/9/03 to allow null updates by Prerna Aggarwal
         ...
         IF P_DE_CHANGE_NOTE IS NULL
         THEN
            v_de_ind.change_note := FALSE;
         ELSIF P_DE_change_note = ' '
         THEN -- Condition added on 9/9/03 to allow null updates by Prerna Aggarwal
   ...
   PROCEDURE SET_RD (P_RETURN_CODE           OUT VARCHAR2,
                     P_ACTION             IN     VARCHAR2,
   ...
   BEGIN
      ...
      THEN                                         --we are inserting a record
         ...
         ELSE
            ...
            --check arc
            IF NOT Sbrext_Common_Routines.valid_arc (P_RD_AC_IDSEQ,
                                                     P_RD_ACH_IDSEQ,
                                                     P_RD_AR_IDSEQ)
            THEN
      ...
      IF P_ACTION = 'UPD'
      THEN                                          --we are updating a record
         IF P_RD_RD_IDSEQ IS NULL
         THEN
            P_RETURN_CODE := 'API_RD_400';   --for updates the ID IS mandatory
            RETURN;
         ELSE
            IF NOT Sbrext_Common_Routines.rd_exists (P_RD_RD_IDSEQ)
            THEN
         ...
         IF (   P_RD_AC_IDSEQ IS NOT NULL
             OR P_RD_ACH_IDSEQ IS NOT NULL
             OR P_RD_AR_IDSEQ IS NOT NULL)
         THEN
      ...
      --Check to see that all VARCHAR2 and  VARCHAR2 parameters have correct length
      IF LENGTH (P_RD_NAME) > Sbrext_Column_Lengths.L_RD_NAME
      THEN
         P_RETURN_CODE := 'API_RD_111'; --Length of name exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_RD_DCTL_NAME) > Sbrext_Column_Lengths.L_RD_DCTL_NAME
      THEN
         P_RETURN_CODE := 'API_RD_112'; --Length of Document Type exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_RD_DOC_TEXT) > Sbrext_Column_Lengths.L_RD_DOC_TEXT
      THEN
         P_RETURN_CODE := 'API_RD_113'; --Length of document text exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_RD_URL) > Sbrext_Column_Lengths.L_RD_URL
      THEN
      ...
      --check to see that charachter strings are valid
      IF NOT Sbrext_Common_Routines.valid_alphanumeric (P_RD_NAME)
      THEN
         P_RETURN_CODE := 'API_RD_130';         -- Name has invalid characters
         RETURN;
      END IF;

      IF NOT Sbrext_Common_Routines.valid_char (P_RD_DOC_TEXT)
      THEN
      ...
      IF (P_RD_AC_IDSEQ) IS NOT NULL
      THEN
         IF NOT Sbrext_Common_Routines.ac_exists (P_RD_AC_IDSEQ)
         THEN
      ...
      IF NOT Sbrext_Common_Routines.dctl_exists (P_RD_DCTL_NAME)
      THEN
         P_RETURN_CODE := 'API_RD_201'; --Document Type not found in the database
         RETURN;
      END IF;

      IF V_CONTE_IDSEQ IS NOT NULL
      THEN
         IF NOT Sbrext_Common_Routines.CONTEXT_EXISTS (v_conte_idseq)
         THEN
      ...
      IF P_RD_ACH_IDSEQ IS NOT NULL
      THEN
         IF NOT Sbrext_Common_Routines.ach_exists (P_RD_ACH_IDSEQ)
         THEN
      ...
      IF P_RD_AR_IDSEQ IS NOT NULL
      THEN
         IF NOT Sbrext_Common_Routines.ar_exists (P_RD_AR_IDSEQ)
         THEN
      ...
      IF P_RD_ORG_IDSEQ IS NOT NULL
      THEN
         IF NOT Sbrext_Common_Routines.org_exists (P_RD_ACH_IDSEQ)
         THEN
      ...
      THEN
         ...
         IF Sbrext_Common_Routines.rd_exists (P_RD_NAME, P_RD_AC_IDSEQ)
         THEN
      ...
      THEN
         ...
         IF P_RD_ORG_IDSEQ IS NULL
         THEN
            v_rd_ind.org_idseq := FALSE;
         ELSIF P_RD_ORG_IDSEQ = ' '
         THEN -- Condition added on 9/9/03 to allow null updates by Prerna Aggarwal
   ...
   PROCEDURE SET_DES (P_RETURN_CODE            OUT VARCHAR2,
                      P_ACTION              IN     VARCHAR2,
   ...
   BEGIN
      ...
      IF P_ACTION = 'INS'
      THEN                                         --we are inserting a record
         IF P_DES_DESIG_IDSEQ IS NOT NULL
         THEN
      ...
      IF P_ACTION = 'UPD'
      THEN                                          --we are updating a record
         IF P_DES_DESIG_IDSEQ IS NULL
         THEN
            P_RETURN_CODE := 'API_DES_400';  --for updates the ID IS mandatory
            RETURN;
         ELSE
            IF NOT Sbrext_Common_Routines.des_exists (P_DES_DESIG_IDSEQ)
            THEN
      ...
      --Check to see that all VARCHAR2 and  VARCHAR2 parameters have correct length
      IF LENGTH (P_DES_NAME) > Sbrext_Column_Lengths.L_DES_NAME
      THEN
         P_RETURN_CODE := 'API_DES_111'; --Length of name exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_DES_DETL_NAME) > Sbrext_Column_Lengths.L_DES_DETL_NAME
      THEN
         P_RETURN_CODE := 'API_DES_112'; --Length of Designation Type exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_DES_LAE_NAME) > Sbrext_Column_Lengths.L_DES_LAE_NAME
      THEN
      ...
      IF NOT Sbrext_Common_Routines.valid_name (P_DES_NAME)
      THEN
      ...
      IF NOT Sbrext_Common_Routines.ac_exists (P_DES_AC_IDSEQ)
      THEN
         P_RETURN_CODE := 'API_DES_200'; --Administered Component not found in the database
         RETURN;
      END IF;

      IF NOT Sbrext_Common_Routines.detl_exists (P_DES_DETL_NAME)
      THEN
         P_RETURN_CODE := 'API_DES_201'; --Desgination Type not found in the database
         RETURN;
      END IF;

      IF NOT Sbrext_Common_Routines.context_exists (P_DES_CONTE_IDSEQ)
      THEN
         P_RETURN_CODE := 'API_DES_203';   --Context not found in the database
         RETURN;
      END IF;

      IF P_DES_LAE_NAME IS NOT NULL
      THEN
         IF NOT Sbrext_Common_Routines.lae_exists (P_DES_LAE_NAME)
         THEN
      ...
      THEN
         ...
         --Name, AC_IDSEQ does not already exist
         IF Sbrext_Common_Routines.des_exists (P_DES_AC_IDSEQ,
                                               P_DES_NAME,
                                               P_DES_DETL_NAME,
                                               P_DES_CONTE_IDSEQ)
         THEN
      ...
      THEN
         ...
         IF P_DES_LAE_NAME IS NULL
         THEN
            v_des_ind.lae_name := FALSE;
         ELSIF P_des_lae_name = ' '
         THEN -- Condition added on 9/9/03 to allow null updates by Prerna Aggarwal
   ...
   PROCEDURE SET_CSCSI (P_RETURN_CODE                  OUT VARCHAR2,
                        P_ACTION                    IN     VARCHAR2,
   ...
   BEGIN
      ...
      IF P_ACTION = 'INS'
      THEN                                         --we are inserting a record
         IF P_CSCSI_CS_CSI_IDSEQ IS NOT NULL
         THEN
      ...
      IF NOT Sbrext_Common_Routines.ac_exists (P_CSCSI_CS_IDSEQ)
      THEN
         P_RETURN_CODE := 'API_CSCSI_201';      --CS NOT found in the database
         RETURN;
      END IF;

      IF NOT Sbrext_Common_Routines.csi_exists (P_CSCSI_CSI_IDSEQ)
      THEN
         P_RETURN_CODE := 'API_CSCSI_202';     --CSI NOT found in the database
         RETURN;
      END IF;

      IF P_CSCSI_P_CS_CSI_IDSEQ IS NOT NULL
      THEN
         IF NOT Sbrext_Common_Routines.cs_csi_exists (P_CSCSI_P_CS_CSI_IDSEQ)
         THEN
      ...
      IF P_CSCSI_LINK_CS_CSI_IDSEQ IS NOT NULL
      THEN
         IF NOT Sbrext_Common_Routines.cs_csi_exists (
      ...
      THEN
         ...
         IF Sbrext_Common_Routines.cs_csi_exists (P_CSCSI_CS_IDSEQ,
                                                  P_CSCSI_CSI_IDSEQ,
   ...
   PROCEDURE SET_ACCSI (P_RETURN_CODE              OUT VARCHAR2,
                        P_ACTION                IN     VARCHAR2,
   ...
   BEGIN
      ...
      IF P_ACTION = 'INS'
      THEN                                         --we are inserting a record
         IF P_ACCSI_AC_CSI_IDSEQ IS NOT NULL
         THEN
         ...
         ELSE
            ...
            IF P_ACCSI_AC_IDSEQ IS NULL
            THEN
               P_RETURN_CODE := 'API_ACCSI_102';       --AC_IDSEQ is mandatory
               RETURN;
            END IF;

            IF P_ACCSI_CS_CSI_IDSEQ IS NULL
            THEN
      ...
      IF P_ACTION = 'DEL'
      THEN                                          --we are deleting a record
         IF P_ACCSI_AC_CSI_IDSEQ IS NULL
         THEN
            P_RETURN_CODE := 'API_ACCSI_400'; --for deleted the ID idmandatory
            RETURN;
         ELSE
            IF NOT Sbrext_Common_Routines.ac_csi_exists (
      ...
      IF NOT Sbrext_Common_Routines.ac_exists (P_ACCSI_AC_IDSEQ)
      THEN
         P_RETURN_CODE := 'API_ACCSI_201'; --Administered Component NOT found in the database
         RETURN;
      END IF;

      IF NOT Sbrext_Common_Routines.cs_csi_exists (P_ACCSI_CS_CSI_IDSEQ)
      THEN
      ...
      THEN
         ...
         IF Sbrext_Common_Routines.ac_csi_exists (P_ACCSI_AC_IDSEQ,
   ...
   PROCEDURE SET_SRC (P_RETURN_CODE            OUT VARCHAR2,
                      P_ACTION              IN     VARCHAR2,
   ...
   BEGIN
      ...
      THEN
         ...
         IF NOT Sbrext_Common_Routines.src_exists (P_SRC_SRC_NAME)
         THEN
      ...
      --Check to see that all VARCHAR2 and  VARCHAR2 parameters have correct length
      IF LENGTH (P_SRC_SRC_NAME) > Sbrext_Column_Lengths.L_SRC_SRC_NAME
      THEN
         P_RETURN_CODE := 'API_SRC_111'; --Length of NAME exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_SRC_DESCRIPTION) > Sbrext_Column_Lengths.L_SRC_DESCRIPTION
      THEN
      ...
      --check to see that charachter strings are valid
      IF NOT Sbrext_Common_Routines.valid_char (P_SRC_SRC_NAME)
      THEN
         P_RETURN_CODE := 'API_SRC_130'; -- Source Name Short Meaning has invalid characters
         RETURN;
      END IF;

      IF P_SRC_DESCRIPTION IS NOT NULL
      THEN
         IF NOT Sbrext_Common_Routines.valid_char (P_SRC_DESCRIPTION)
         THEN
      ...
      THEN
         --check to see that Short Name Does not already Exist
         IF Sbrext_Common_Routines.src_exists (P_SRC_SRC_NAME)
         THEN
   ...
   PROCEDURE SET_TS (P_RETURN_CODE           OUT VARCHAR2,
                     P_ACTION             IN     VARCHAR2,
   ...
   BEGIN
      ...
      IF P_ACTION = 'UPD'
      THEN                                         --we are inserting a record
         IF P_TS_TS_IDSEQ IS NULL
         THEN
            P_RETURN_CODE := 'API_TS_400';   --for updates the ID is mandatory
            RETURN;
         ELSE
            IF NOT Sbrext_Common_Routines.ts_exists (P_TS_TS_IDSEQ)
            THEN
      ...
      IF LENGTH (P_TS_TSTL_NAME) > Sbrext_Column_Lengths.L_TS_TSTL_NAME
      THEN
         P_RETURN_CODE := 'API_TS_113'; --Length of TS Type exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_TS_TS_TEXT) > Sbrext_Column_Lengths.L_TS_TS_TEXT
      THEN
      ...
      --check to see that charachter strings are valid
      IF NOT Sbrext_Common_Routines.valid_char (P_TS_TSTL_NAME)
      THEN
         P_RETURN_CODE := 'API_TS_133'; -- TEXT STRING Type has invalid characters
         RETURN;
      END IF;

      IF NOT Sbrext_Common_Routines.valid_char (P_TS_TS_TEXT)
      THEN
      ...
      IF NOT Sbrext_Common_Routines.tstl_exists (P_TS_TSTL_NAME)
      THEN
         P_RETURN_CODE := 'API_TS_200'; --Text String Type not found in the database
         RETURN;
      END IF;

      IF NOT Sbrext_Common_Routines.ac_exists (P_TS_QC_IDSEQ)
      THEN
   ...
   PROCEDURE SET_ACSRC (P_RETURN_CODE               OUT VARCHAR2,
                        P_ACTION                 IN     VARCHAR2,
   ...
   BEGIN
      ...
      IF P_ACTION = 'DEL'
      THEN                                         --we are inserting a record
         IF P_ACSRC_ACS_IDSEQ IS NULL
         THEN
            P_RETURN_CODE := 'API_ACSRC_400'; --for updates the ID is mandatory
            RETURN;
         ELSE
            IF NOT Sbrext_Common_Routines.acsrc_exists (P_ACSRC_ACS_IDSEQ)
            THEN
      ...
      --Check to see that all VARCHAR2 and CHAR parameters have correct length
      IF LENGTH (P_ACSRC_SRC_NAME) > Sbrext_Column_Lengths.L_ACSRC_SRC_NAME
      THEN
      ...
      IF NOT Sbrext_Common_Routines.src_exists (P_ACSRC_SRC_NAME)
      THEN
         P_RETURN_CODE := 'API_ACSRC_200'; --Source Name not found in the database
         RETURN;
      END IF;

      IF NOT Sbrext_Common_Routines.ac_exists (P_ACSRC_AC_IDSEQ)
      THEN
      ...
      THEN
         ...
         IF Sbrext_Common_Routines.acsrc_exists (P_ACSRC_AC_IDSEQ,
                                                 P_ACSRC_SRC_NAME)
         THEN
   ...
   PROCEDURE SET_VPSRC (P_RETURN_CODE               OUT VARCHAR2,
                        P_ACTION                 IN     VARCHAR2,
   ...
   BEGIN
      ...
      IF P_ACTION = 'DEL'
      THEN                                         --we are inserting a record
         IF P_VPSRC_VPS_IDSEQ IS NULL
         THEN
            P_RETURN_CODE := 'API_VPSRC_400'; --for updates the ID is mandatory
            RETURN;
         ELSE
            IF NOT Sbrext_Common_Routines.vpsrc_exists (P_VPSRC_VPS_IDSEQ)
            THEN
      ...
      --Check to see that all VARCHAR2 and CHAR parameters have correct length
      IF LENGTH (P_VPSRC_SRC_NAME) > Sbrext_Column_Lengths.L_VPSRC_SRC_NAME
      THEN
      ...
      IF NOT Sbrext_Common_Routines.src_exists (P_VPSRC_SRC_NAME)
      THEN
         P_RETURN_CODE := 'API_VPSRC_200'; --Source Name not found in the database
         RETURN;
      END IF;

      IF NOT Sbrext_Common_Routines.vd_pvs_exists (P_VPSRC_VP_IDSEQ)
      THEN
      ...
      THEN
         --check to see that record  does not already
         IF Sbrext_Common_Routines.vpsrc_exists (P_VPSRC_VP_IDSEQ,
                                                 P_VPSRC_SRC_NAME)
         THEN
   ...
   BEGIN
      ...
      THEN                                         --we are inserting a record
         ...
         ELSE
            ...
            IF P_QC_LATEST_VERSION_IND IS NULL
            THEN
               P_QC_LATEST_VERSION_IND := 'No';
            END IF;

            IF P_QC_QTL_NAME IS NULL
            THEN
            ...
            /*    IF (P_QC_DE_IDSEQ is null and P_QC_VP_IDSEQ is null and P_QC_QC_MATCH_IDSEQ is null) THEN
                ...
                END IF;*/
            --check the arc
            IF NOT Sbrext_Common_Routines.valid_arc (P_QC_DE_IDSEQ,
                                                     P_QC_VP_IDSEQ,
                                                     P_QC_QC_MATCH_IDSEQ)
            THEN
      ...
      THEN                                          --we are updating a record
         ...
         IF     (   P_QC_PREFERRED_NAME IS NOT NULL
                 OR P_QC_CONTE_IDSEQ IS NOT NULL)
            AND v_asl_name = 'RELEASED'
         THEN
         ...
         IF NOT Sbrext_Common_Routines.ac_exists (P_QC_QC_IDSEQ)
         THEN
         ...
         --check the arc
         IF NOT (   P_QC_DE_IDSEQ = ' '
                 OR P_QC_VP_IDSEQ = ' '
                 OR P_QC_QC_MATCH_IDSEQ = ' ')
         THEN
            IF NOT Sbrext_Common_Routines.valid_arc (P_QC_DE_IDSEQ,
                                                     P_QC_VP_IDSEQ,
                                                     P_QC_QC_MATCH_IDSEQ)
      ...
      IF P_QC_LATEST_VERSION_IND IS NOT NULL
      THEN
         IF P_QC_LATEST_VERSION_IND NOT IN ('Yes', 'No')
         THEN
      ...
      IF P_QC_HIGHLIGHT_IND IS NOT NULL
      THEN
         IF P_QC_HIGHLIGHT_IND NOT IN ('Yes', 'No')
         THEN
      ...
      --Check to see that all VARCHAR2 and CHAR parameters have correct length
      IF LENGTH (P_QC_PREFERRED_NAME) >
            Sbrext_Column_Lengths.L_QC_PREFERRED_NAME
      THEN
         P_RETURN_CODE := 'API_QC_111'; --Length of preferred_name exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_QC_PREFERRED_DEFINITION) >
            Sbrext_Column_Lengths.L_QC_PREFERRED_DEFINITION
      THEN
         P_RETURN_CODE := 'API_QC_112'; --Length of Preferrede Definition exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_QC_QTL_NAME) > Sbrext_Column_Lengths.L_QC_QTL_NAME
      THEN
         P_RETURN_CODE := 'API_QC_113'; --Length of QTL_name exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_QC_QC_IDENTIFIER) >
            Sbrext_Column_Lengths.L_QC_QC_IDENTIFIER
      THEN
         P_RETURN_CODE := 'API_QC_114'; --Length of QC_IDENTIFIER exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_QC_ASL_NAME) > Sbrext_Column_Lengths.L_QC_ASL_NAME
      THEN
         P_RETURN_CODE := 'API_QC_115'; --Length of asl_name exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_QC_CHANGE_NOTE) > Sbrext_Column_Lengths.L_QC_CHANGE_NOTE
      THEN
         P_RETURN_CODE := 'API_QC_116'; --Length of change_note exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_QC_LONG_NAME) > Sbrext_Column_Lengths.L_QC_LONG_NAME
      THEN
         P_RETURN_CODE := 'API_QC_117'; --Length of Long_name exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_QC_MATCH_IND) > Sbrext_Column_Lengths.L_QC_QC_MATCH_IND
      THEN
         P_RETURN_CODE := 'API_QC_118'; --Length of Match IND exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_QC_REVIEWER_FEEDBACK_ACT) >
            Sbrext_Column_Lengths.L_QC_REVIEWER_FEEDBACK_ACT
      THEN
         P_RETURN_CODE := 'API_QC_119'; --Length of feedback action exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_QC_NEW_QC_IND) > Sbrext_Column_Lengths.L_QC_NEW_QC_IND
      THEN
         P_RETURN_CODE := 'API_QC_120'; --Length of New QC IND exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_QC_REVIEWER_FEEDBACK_EXT) >
            Sbrext_Column_Lengths.L_QC_REVIEWER_FEEDBACK_EXT
      THEN
         P_RETURN_CODE := 'API_QC_122'; --Length of external feedback exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_QC_SYSTEM_MSGS) > Sbrext_Column_Lengths.L_QC_SYSTEM_MSGS
      THEN
         P_RETURN_CODE := 'API_QC_123'; --Length of approved_by exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_QC_REVIEWER_FEEDBACK_INT) >
            Sbrext_Column_Lengths.L_QC_REVIEWER_FEEDBACK_INT
      THEN
         P_RETURN_CODE := 'API_QC_124'; --Length of internal feedback exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_QC_REVIEWED_BY) > Sbrext_Column_Lengths.L_QC_REVIEWED_BY
      THEN
         P_RETURN_CODE := 'API_QC_125'; --Length of reviewed_by exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_QC_APPROVED_BY) > Sbrext_Column_Lengths.L_QC_APPROVED_BY
      THEN
         P_RETURN_CODE := 'API_QC_126'; --Length of approved_by exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_QC_SUB_LONG_NAME) >
            Sbrext_Column_Lengths.L_QC_SUB_LONG_NAME
      THEN
         P_RETURN_CODE := 'API_QC_127'; --Length of submitted long name exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_QC_GROUP_COMMENTS) >
            Sbrext_Column_Lengths.L_QC_GROUP_COMMENTS
      THEN
         P_RETURN_CODE := 'API_QC_128'; --Length of approved_by exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_QC_DE_LONG_NAME) > Sbrext_Column_Lengths.L_DE_LONG_NAME
      THEN
         P_RETURN_CODE := 'API_QC_150'; --Length of de_lonmg_name exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_QC_DEC_LONG_NAME) > Sbrext_Column_Lengths.L_DEC_LONG_NAME
      THEN
         P_RETURN_CODE := 'API_QC_151'; --Length of dec_long_name exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_QC_VD_LONG_NAME) > Sbrext_Column_Lengths.L_VD_LONG_NAME
      THEN
      ...
      --check to see that charachter strings are valid
      IF NOT Sbrext_Common_Routines.valid_alphanumeric (P_QC_PREFERRED_NAME)
      THEN
      ...
      /*IF NOT Sbrext_Common_Routines.valid_char(P_QC_PREFERRED_DEFINITION) THEN
        ...
        RETURN;
      END IF;*/
      IF NOT Sbrext_Common_Routines.valid_char (P_QC_LONG_NAME)
      THEN
         P_RETURN_CODE := 'API_QC_134'; -- Value Domain Long Name has invalid characters
         RETURN;
      END IF;

      IF P_QC_REVIEWER_FEEDBACK_INT IS NOT NULL
      THEN
         IF NOT Sbrext_Common_Routines.valid_char (
                   P_QC_REVIEWER_FEEDBACK_INT)
         THEN
      ...
      IF P_QC_REVIEWER_FEEDBACK_ACT IS NOT NULL
      THEN
         IF NOT Sbrext_Common_Routines.valid_char (
                   P_QC_REVIEWER_FEEDBACK_ACT)
         THEN
      ...
      IF P_QC_REVIEWER_FEEDBACK_EXT IS NOT NULL
      THEN
         IF NOT Sbrext_Common_Routines.valid_char (
                   P_QC_REVIEWER_FEEDBACK_EXT)
         THEN
      ...
      IF P_QC_SUB_LONG_NAME IS NOT NULL
      THEN
         IF NOT Sbrext_Common_Routines.valid_char (P_QC_SUB_LONG_NAME)
         THEN
      ...
      IF P_QC_GROUP_COMMENTS IS NOT NULL
      THEN
         IF NOT Sbrext_Common_Routines.valid_char (P_QC_GROUP_COMMENTS)
         THEN
      ...
      --check to see that Foreign keys already exist in the database
      IF P_QC_CONTE_IDSEQ IS NOT NULL
      THEN
         IF NOT Sbrext_Common_Routines.context_exists (P_QC_CONTE_IDSEQ)
         THEN
      ...
      IF P_QC_PROTO_IDSEQ IS NOT NULL
      THEN
         IF NOT Sbrext_Common_Routines.ac_exists (P_QC_PROTO_IDSEQ)
         THEN
      ...
      IF P_QC_ASL_NAME IS NOT NULL
      THEN
         IF NOT Sbrext_Common_Routines.workflow_exists (P_QC_ASL_NAME)
         THEN
      ...
      IF (P_QC_VP_IDSEQ IS NOT NULL)
      THEN
         -- dbms_output.put_line('p_qc_vp_idseq is not null ='||nvl(P_QC_VP_IDSEQ,'null')||
         -- 'length='||length(P_QC_VP_IDSEQ));
         IF (P_QC_VP_IDSEQ <> ' ')
         THEN
            IF NOT Sbrext_Common_Routines.VD_PVS_EXISTS (P_QC_VP_IDSEQ)
            THEN
               -- dyu 1/23/2003
               DBMS_OUTPUT.put_line (
                  'in api_qc_203:P_QC_VP_IDSEQ= ' || P_QC_VP_IDSEQ);
      ...
      IF P_QC_QTL_NAME IS NOT NULL
      THEN
         IF NOT Sbrext_Common_Routines.qtl_exists (P_QC_QTL_NAME)
         THEN
      ...
      IF (P_QC_DE_IDSEQ IS NOT NULL)
      THEN
         IF P_QC_DE_IDSEQ <> ' '
         THEN
            IF NOT Sbrext_Common_Routines.ac_exists (P_QC_DE_IDSEQ)
            THEN
      ...
      IF P_QC_QCDL_NAME IS NOT NULL
      THEN
         IF NOT Sbrext_Common_Routines.qcdl_exists (P_QC_QCDL_NAME)
         THEN
      ...
      IF P_QC_REVIEWER_FEEDBACK_EXT IS NOT NULL THEN
        ...
        END IF;
      END IF;*/
      --This has been commented out because these should not be forign keys.
      IF (P_QC_REVIEWER_FEEDBACK_ACT IS NOT NULL)
      THEN
         IF P_QC_REVIEWER_FEEDBACK_ACT <> ' '
         THEN
            IF NOT Sbrext_Common_Routines.rf_exists (
                      P_QC_REVIEWER_FEEDBACK_ACT,
                      'REVIEWER ACTION')
            THEN
      ...
      --check to see that begin data and end date are valid dates
      IF (P_QC_BEGIN_DATE IS NOT NULL)
      THEN
         Sbrext_Common_Routines.valid_date (P_RETURN_CODE,
                                            P_QC_BEGIN_DATE,
                                            v_begin_date);
      ...
      IF (P_QC_REVIEWED_DATE IS NOT NULL)
      THEN
         Sbrext_Common_Routines.valid_date (P_RETURN_CODE,
                                            P_QC_REVIEWED_DATE,
                                            v_reviewed_date);
      ...
      IF (P_QC_APPROVED_DATE IS NOT NULL)
      THEN
         Sbrext_Common_Routines.valid_date (P_RETURN_CODE,
                                            P_QC_APPROVED_DATE,
                                            v_approved_date);
      ...
      IF (P_QC_BEGIN_DATE IS NOT NULL AND P_QC_END_DATE IS NOT NULL)
      THEN
         IF (v_end_date < v_begin_date)
         THEN
            ...
            RETURN;
         END IF;
      ELSIF (P_QC_END_DATE IS NOT NULL AND P_QC_BEGIN_DATE IS NULL)
      THEN
      ...
      THEN
         ...
         --Preferred Name, Context and Version does not already exist
         IF Sbrext_Common_Routines.ac_exists (P_QC_PREFERRED_NAME,
                                              P_QC_CONTE_IDSEQ,
                                              P_QC_VERSION,
                                              'QUEST_CONTENT')
         THEN
         ...
         --Check to see if prior versions alresdy exist
         IF Sbrext_Common_Routines.ac_version_exists (P_QC_PREFERRED_NAME,
                                                      P_QC_CONTE_IDSEQ,
                                                      'QUEST_CONTENT')
         THEN                                 -- we are creating a new version
            v_new_version := TRUE;
            v_ac :=
               Sbrext_Common_Routines.get_version_ac (P_QC_PREFERRED_NAME,
                                                      P_QC_CONTE_IDSEQ,
                                                      'QUEST_CONTENT');
         END IF;
         ...
         BEGIN
            ...
            sbrext_common_routines.SET_MULTI_PROTO (P_QC_PROTO_IDSEQ,
                                                    P_QC_QC_IDSEQ);
            meta_global_pkg.transaction_type := NULL;
         EXCEPTION
            WHEN OTHERS
         ...
         --If LATEST_VERSION_IND is'Yes' then update so that all other versions have the indicator set to 'No'
         IF (P_QC_LATEST_VERSION_IND = 'Yes')
         THEN
            Sbrext_Common_Routines.set_ac_lvi (P_RETURN_CODE,
                                               P_QC_QC_IDSEQ,
                                               'QUEST_CONTENT');
      ...
      THEN
         ...
         IF P_QC_VERSION IS NOT NULL
         THEN                    -- 23-Jul-2003, W. Ver Hoef - added condition
            IF v_version <> P_QC_VERSION
            THEN
         ...
         IF P_QC_MATCH_IND IS NULL
         THEN
            v_qc_ind.match_ind := FALSE;
         ELSE
            v_qc_rec.match_ind := P_QC_MATCH_IND;

            IF v_qc_rec.match_ind = ' '
            THEN
         ...
         IF P_QC_VP_IDSEQ IS NULL
         THEN
            v_qc_ind.vp_idseq := FALSE;
         ELSE
            v_qc_rec.vp_idseq := P_QC_VP_IDSEQ;

            IF v_qc_rec.vp_idseq = ' '
            THEN
         ...
         IF P_QC_QC_MATCH_IDSEQ IS NULL
         THEN
            v_qc_ind.qc_match_idseq := FALSE;
         ELSE
            v_qc_rec.qc_match_idseq := P_QC_QC_MATCH_IDSEQ;

            IF v_qc_rec.qc_match_idseq = ' '
            THEN
         ...
         IF P_QC_REVIEWER_FEEDBACK_INT IS NULL
         THEN
            v_qc_ind.reviewer_feedback_internal := FALSE;
         ELSE
            v_qc_rec.reviewer_feedback_internal := P_QC_REVIEWER_FEEDBACK_INT;
         ...
         IF P_QC_GROUP_COMMENTS IS NULL
         THEN
            v_qc_ind.group_comments := FALSE;
         ELSE
            v_qc_rec.group_comments := P_QC_GROUP_COMMENTS;
         ...
         IF P_QC_VD_IDSEQ IS NULL
         THEN
            v_qc_ind.dn_vd_idseq := FALSE;
         ELSE
            v_qc_rec.dn_vd_idseq := P_QC_VD_IDSEQ;

            IF v_qc_rec.dn_vd_idseq = ' '
            THEN
         ...
         IF P_QC_DE_IDSEQ IS NULL
         THEN
            v_qc_ind.de_idseq := FALSE;
         ELSE
            v_qc_rec.de_idseq := P_QC_DE_IDSEQ;

            IF v_qc_rec.de_idseq = ' '
            THEN
         ...
         IF P_QC_NEW_QC_IND IS NULL
         THEN
            v_qc_ind.new_qc_ind := FALSE;
         ELSE
            v_qc_rec.new_qc_ind := P_QC_NEW_QC_IND;

            IF v_qc_rec.new_qc_ind = ' '
            THEN
         ...
         ELSE
            ...
            v_qc_ind.highlight_ind := TRUE;
            DBMS_OUTPUT.put_line (
               'highlight_ind= ' || v_qc_rec.highlight_ind);
         END IF;

         IF P_QC_SYSTEM_MSGS IS NULL
         THEN
            v_qc_ind.system_msgs := FALSE;
         ELSE
            v_qc_rec.system_msgs := P_QC_SYSTEM_MSGS;
         ...
         IF P_QC_REVIEWER_FEEDBACK_ACT IS NULL
         THEN
            v_qc_ind.reviewer_feedback_action := FALSE;
         ELSE
            v_qc_rec.reviewer_feedback_action := P_QC_REVIEWER_FEEDBACK_ACT;

            IF v_qc_rec.reviewer_feedback_action = ' '
            THEN
         ...
         IF P_QC_CDE_DICTIONARY_ID IS NULL
         THEN
            v_qc_ind.cde_dictionary_id := FALSE;
         ELSE
         ...
         IF P_QC_REVIEWER_FEEDBACK_EXT IS NULL
         THEN
            v_qc_ind.reviewer_feedback_external := FALSE;
         ELSE
            v_qc_rec.reviewer_feedback_external := P_QC_REVIEWER_FEEDBACK_EXT;
   ...
   PROCEDURE SET_REL (P_RETURN_CODE            OUT VARCHAR2,
                      P_ACTION              IN     VARCHAR2,
   ...
   BEGIN
      ...
      IF    P_ACTION NOT IN ('INS', 'DEL', 'UPD')
         OR (P_ACTION = 'UPD' AND P_REL_TABLE <> 'QC_RECS_EXT')
      THEN
      ...
      IF (P_ACTION = 'INS')
      THEN
         IF P_REL_REL_IDSEQ IS NOT NULL
         THEN
         ...
         ELSE
            ...
            IF (P_REL_TABLE = 'QC_RECS_EXT' AND P_REL_DISPLAY_ORDER IS NULL)
            THEN
      ...
      IF P_ACTION = 'DEL'
      THEN                                          --we are deleting a record
         IF P_REL_REL_IDSEQ IS NULL
         THEN
            P_RETURN_CODE := 'API_REL_400';  --for deleted the ID is mandatory
            RETURN;
         END IF;

         IF NOT Sbrext_Common_Routines.rel_exists (P_REL_TABLE,
                                                   P_REL_REL_IDSEQ)
         THEN
         ...
         IF P_REL_TABLE = 'DE_RECS'
         THEN
            v_rel_de_pk.de_rec_idseq := P_REL_REL_IDSEQ;

            SELECT ROWID
              INTO v_rel_de_pk.the_rowid
              FROM de_recs_view
            ...
            EXCEPTION
               ...
               THEN
                  ...
                  RETURN;
            END;
         ELSIF P_REL_TABLE = 'DEC_RECS'
         THEN
            v_rel_dec_pk.dec_rec_idseq := P_REL_REL_IDSEQ;

            SELECT ROWID
              INTO v_rel_dec_pk.the_rowid
              FROM dec_recs_view
            ...
            EXCEPTION
               ...
               THEN
                  ...
                  RETURN;
            END;
         ELSIF P_REL_TABLE = 'VD_RECS'
         THEN
            v_rel_vd_pk.vd_rec_idseq := P_REL_REL_IDSEQ;

            SELECT ROWID
              INTO v_rel_vd_pk.the_rowid
              FROM vd_recs_view
            ...
            EXCEPTION
               ...
               THEN
                  ...
                  RETURN;
            END;
         ELSIF P_REL_TABLE = 'CS_RECS'
         THEN
            v_rel_cs_pk.cs_rec_idseq := P_REL_REL_IDSEQ;

            SELECT ROWID
              INTO v_rel_cs_pk.the_rowid
              FROM cs_recs_view
            ...
            EXCEPTION
               ...
               THEN
                  ...
                  RETURN;
            END;
         ELSIF P_REL_TABLE = 'CSI_RECS'
         THEN
            v_rel_csi_pk.csi_rec_idseq := P_REL_REL_IDSEQ;

            SELECT ROWID
              INTO v_rel_csi_pk.the_rowid
              FROM csi_recs_view
            ...
            EXCEPTION
               ...
               THEN
                  ...
                  RETURN;
            END;
         ELSIF P_REL_TABLE = 'QC_RECS_EXT'
         THEN
            v_rel_qce_pk.qr_idseq := P_REL_REL_IDSEQ;

            SELECT ROWID
              INTO v_rel_qce_pk.the_rowid
      ...
      IF P_ACTION = 'UPD' AND P_REL_TABLE = 'QC_RECS_EXT'
      THEN                                          --we are updating a record
         IF P_REL_REL_IDSEQ IS NULL
         THEN
         ...
         IF NOT Sbrext_Common_Routines.rel_exists (P_REL_TABLE,
                                                   P_REL_REL_IDSEQ)
         THEN                            --if relationship does not exist then
            P_RETURN_CODE := 'API_REL_005';                   --REL  NOT found
            RETURN;
         END IF;

         IF P_REL_P_IDSEQ IS NOT NULL OR P_REL_C_IDSEQ IS NOT NULL
         THEN
            P_RETURN_CODE := 'API_REL_QCE_100'; --The parent and the child can not be updated
            RETURN;
         END IF;

         IF P_REL_DISPLAY_ORDER IS NULL
         THEN
      ...
      IF LENGTH (P_REL_RL_NAME) > Sbrext_Column_Lengths.L_REL_RL_NAME
      THEN
      ...
      IF P_REL_TABLE IN ('DE_RECS',
                         'DEC_RECS',
                         'VD_RECS',
                         'CS_RECS',
                         'QC_RECS_EXT')
      THEN
         IF NOT Sbrext_Common_Routines.valid_ac (P_REL_TABLE, P_REL_P_IDSEQ)
         THEN
            P_RETURN_CODE := 'API_REL_200';          -- parent does not exist;
            RETURN;                                        --parent is invalie
         END IF;

         IF NOT Sbrext_Common_Routines.valid_ac (P_REL_TABLE, P_REL_C_IDSEQ)
         THEN
      ...
      ELSIF P_REL_TABLE = 'CSI_RECS'
      THEN
         IF NOT Sbrext_Common_Routines.csi_exists (P_REL_P_IDSEQ)
         THEN
            P_RETURN_CODE := 'API_REL_200';          -- parent does not exist;
            RETURN;
         END IF;

         IF NOT Sbrext_Common_Routines.csi_exists (P_REL_C_IDSEQ)
         THEN
            P_RETURN_CODE := 'API_REL_201';           -- child does not exist;
            RETURN;
         END IF;
      ...
      IF NOT Sbrext_Common_Routines.rl_exists (P_REL_RL_NAME)
      THEN
      ...
      THEN
         ...
         IF P_REL_TABLE IN ('DE_RECS', 'DEC_RECS', 'VD_RECS')
         THEN
            IF Sbrext_Common_Routines.rel_exists (P_REL_TABLE,
                                                  P_REL_P_IDSEQ,
                                                  P_REL_C_IDSEQ)
            THEN
   ...
   BEGIN
      ...
      THEN
         ...
         DBMS_OUTPUT.put_line (
            'finished ind about to call Cg$protocols_View_Ext.ins...');
         ...
         EXCEPTION
            ...
            THEN
               ...
               DBMS_OUTPUT.put_line ('SQLCODE = ' || SQLCODE);
               DBMS_OUTPUT.put_line (
                  'Error inserting Protocol ' || SUBSTR (SQLERRM, 1, 200));
   ...
   PROCEDURE SET_CD_VMS (P_RETURN_CODE              OUT VARCHAR2,
                         P_ACTION                IN     VARCHAR2,
   ...
   BEGIN
      ...
      IF P_ACTION = 'INS'
      THEN                                         --we are inserting a record
         IF P_CDVMS_CV_IDSEQ IS NOT NULL
         THEN
         ...
         ELSE
            ...
            IF P_CDVMS_CD_IDSEQ IS NULL
            THEN
               P_RETURN_CODE := 'API_CDVMS_102'; --VD_IDSEQ cannot be null here
               RETURN;
            END IF;

            IF P_CDVMS_SHORT_MEANING IS NULL
            THEN
      ...
      IF NOT Sbrext_Common_Routines.ac_exists (P_CDVMS_CD_IDSEQ)
      THEN
      ...
      IF NOT Sbrext_Common_Routines.vm_exists (
                p_short_meaning   => P_CDVMS_SHORT_MEANING)
      THEN
         P_RETURN_CODE := 'API_CDVMS_202'; --Value meaning not found in the database
         RETURN;
      END IF;

      IF Sbrext_Common_Routines.cd_vm_exists (
            p_cd_idseq        => P_CDVMS_CD_IDSEQ,
            p_short_meaning   => P_CDVMS_SHORT_MEANING)
      THEN
   ...
   BEGIN
      ...
      IF NOT Sbrext_Common_Routines.vm_exists (p_vm_idseq => P_CDVMS_VM_IDSEQ)
      THEN
         P_RETURN_CODE := 'API_CDVMS_202'; --Value meaning not found in the database
         RETURN;
      END IF;
   ...
   PROCEDURE SET_PROPERTY (P_RETURN_CODE                    OUT VARCHAR2,
                           P_ACTION                      IN     VARCHAR2,
   ...
   BEGIN
      ...
      IF P_ACTION = 'DEL'
      THEN                                          --we are deleting a record
         IF P_PROP_IDSEQ IS NULL
         THEN
            P_RETURN_CODE := 'API_PROP_400';  --for deleted the ID idmandatory
            RETURN;
         ELSE
            IF NOT Sbrext_Common_Routines.ac_exists (P_PROP_IDSEQ)
            THEN
      ...
      THEN                                          --we are updating a record
         ...
         IF     (   P_PROP_PREFERRED_NAME IS NOT NULL
                 OR P_PROP_CONTE_IDSEQ IS NOT NULL)
            AND v_asl_name = 'RELEASED'
         THEN
      ...
      IF P_PROP_LATEST_VERSION_IND IS NOT NULL
      THEN
         IF P_PROP_LATEST_VERSION_IND NOT IN ('Yes', 'No')
         THEN
      ...
      --Check to see that all VARCHAR2 and  VARCHAR2 parameters have correct length
      IF LENGTH (P_PROP_PREFERRED_NAME) >
            Sbrext_Column_Lengths.L_PROP_PREFERRED_NAME
      THEN
         P_RETURN_CODE := 'API_PROP_111'; --Length of preferred_name exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_PROP_PREFERRED_DEFINITION) >
            Sbrext_Column_Lengths.L_PROP_PREFERRED_DEFINITION
      THEN
         P_RETURN_CODE := 'API_PROP_113'; --Length of Preferred_definition exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_PROP_LONG_NAME) > Sbrext_Column_Lengths.L_PROP_LONG_NAME
      THEN
         P_RETURN_CODE := 'API_PROP_114'; --Length of Long_name exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_PROP_ASL_NAME) > Sbrext_Column_Lengths.L_PROP_ASL_NAME
      THEN
         P_RETURN_CODE := 'API_PROP_115'; --Length of asl_name exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_PROP_CHANGE_NOTE) >
            Sbrext_Column_Lengths.L_PROP_CHANGE_NOTE
      THEN
      ...
      IF NOT Sbrext_Common_Routines.valid_char (P_PROP_PREFERRED_DEFINITION)
      THEN
         P_RETURN_CODE := 'API_PROP_133'; -- Data Element Preferred Definition has invalid characters
         RETURN;
      END IF;

      IF NOT Sbrext_Common_Routines.valid_char (P_PROP_LONG_NAME)
      THEN
      ...
      --check to see that Context, Workflow Status, Data Element Concept, Value Domain already exist in the database
      IF P_PROP_CONTE_IDSEQ IS NOT NULL
      THEN
         IF NOT Sbrext_Common_Routines.context_exists (P_PROP_CONTE_IDSEQ)
         THEN
      ...
      IF P_PROP_ASL_NAME IS NOT NULL
      THEN
         IF NOT Sbrext_Common_Routines.workflow_exists (P_PROP_ASL_NAME)
         THEN
      ...
      --check to see that begin data and end date are valid dates
      IF (P_PROP_BEGIN_DATE IS NOT NULL)
      THEN
         Sbrext_Common_Routines.valid_date (P_RETURN_CODE,
                                            P_PROP_BEGIN_DATE,
                                            v_begin_date);
      ...
      IF (P_PROP_END_DATE IS NOT NULL)
      THEN
         Sbrext_Common_Routines.valid_date (P_RETURN_CODE,
                                            P_PROP_END_DATE,
                                            v_end_date);
      ...
      IF (P_PROP_BEGIN_DATE IS NOT NULL AND P_PROP_END_DATE IS NOT NULL)
      THEN
         IF (v_end_date < v_begin_date)
         THEN
            ...
            RETURN;
         END IF;
      ELSIF (P_PROP_END_DATE IS NOT NULL AND P_PROP_BEGIN_DATE IS NULL)
      THEN
      ...
      THEN
         ...
         IF P_PROP_VERSION IS NULL
         THEN
            P_PROP_VERSION := 1;
         END IF;

         IF P_PROP_ASL_NAME IS NULL
         THEN
            -- 19-Mar-2004, W. Ver Hoef - substituted UNASSIGNED with function call below
      ...
      THEN
         ...
         IF P_PROP_LATEST_VERSION_IND IS NULL
         THEN
            v_prop_ind.latest_version_ind := FALSE;
         ELSE
            v_prop_rec.latest_version_ind := P_PROP_LATEST_VERSION_IND;
         ...
         IF P_PROP_DEFINITION_SOURCE IS NULL
         THEN
   ...
   PROCEDURE SET_OBJECT_CLASS (P_RETURN_CODE                  OUT VARCHAR2,
                               P_ACTION                    IN     VARCHAR2,
   ...
   BEGIN
      ...
      IF P_ACTION = 'DEL'
      THEN                                          --we are deleting a record
         IF P_oc_IDSEQ IS NULL
         THEN
            P_RETURN_CODE := 'API_OC_400';    --for deleted the ID idmandatory
            RETURN;
         ELSE
            IF NOT Sbrext_Common_Routines.ac_exists (P_oc_IDSEQ)
            THEN
      ...
      THEN                                          --we are updating a record
         ...
         IF     (   P_oc_PREFERRED_NAME IS NOT NULL
                 OR P_oc_CONTE_IDSEQ IS NOT NULL)
            AND v_asl_name = 'RELEASED'
         THEN
      ...
      IF P_oc_LATEST_VERSION_IND IS NOT NULL
      THEN
         IF P_oc_LATEST_VERSION_IND NOT IN ('Yes', 'No')
         THEN
      ...
      --Check to see that all VARCHAR2 and  VARCHAR2 parameters have correct length
      IF LENGTH (P_oc_PREFERRED_NAME) >
            Sbrext_Column_Lengths.L_oc_PREFERRED_NAME
      THEN
         P_RETURN_CODE := 'API_OC_111'; --Length of preferred_name exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_oc_PREFERRED_DEFINITION) >
            Sbrext_Column_Lengths.L_oc_PREFERRED_DEFINITION
      THEN
         P_RETURN_CODE := 'API_OC_113'; --Length of Preferred_definition exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_oc_LONG_NAME) > Sbrext_Column_Lengths.L_oc_LONG_NAME
      THEN
         P_RETURN_CODE := 'API_OC_114'; --Length of Long_name exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_oc_ASL_NAME) > Sbrext_Column_Lengths.L_oc_ASL_NAME
      THEN
         P_RETURN_CODE := 'API_OC_115'; --Length of asl_name exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_oc_CHANGE_NOTE) > Sbrext_Column_Lengths.L_oc_CHANGE_NOTE
      THEN
      ...
      --check to see that charachter strings are valid
      IF NOT Sbrext_Common_Routines.valid_alphanumeric (P_oc_PREFERRED_NAME)
      THEN
         P_RETURN_CODE := 'API_OC_130'; -- Data Element Preferred Name has invalid Ccharacters
         RETURN;
      END IF;

      IF NOT Sbrext_Common_Routines.valid_char (P_oc_PREFERRED_DEFINITION)
      THEN
         P_RETURN_CODE := 'API_OC_133'; -- Data Element Preferred Definition has invalid characters
         RETURN;
      END IF;

      IF NOT Sbrext_Common_Routines.valid_char (P_oc_LONG_NAME)
      THEN
      ...
      --check to see that Context, Workflow Status, Data Element Concept, Value Domain already exist in the database
      IF P_oc_CONTE_IDSEQ IS NOT NULL
      THEN
         IF NOT Sbrext_Common_Routines.context_exists (P_oc_CONTE_IDSEQ)
         THEN
      ...
      IF P_oc_ASL_NAME IS NOT NULL
      THEN
         IF NOT Sbrext_Common_Routines.workflow_exists (P_oc_ASL_NAME)
         THEN
      ...
      --check to see that begin data and end date are valid dates
      IF (P_oc_BEGIN_DATE IS NOT NULL)
      THEN
         Sbrext_Common_Routines.valid_date (P_RETURN_CODE,
                                            P_oc_BEGIN_DATE,
                                            v_begin_date);
      ...
      IF (P_oc_BEGIN_DATE IS NOT NULL AND P_oc_END_DATE IS NOT NULL)
      THEN
         IF (v_end_date < v_begin_date)
         THEN
            ...
            RETURN;
         END IF;
      ELSIF (P_oc_END_DATE IS NOT NULL AND P_oc_BEGIN_DATE IS NULL)
      THEN
      ...
      THEN
         ...
         IF P_oc_VERSION IS NULL
         THEN
            P_oc_VERSION := 1;
         END IF;

         IF P_oc_ASL_NAME IS NULL
         THEN
            -- 19-Mar-2004, W. Ver Hoef - substituted UNASSIGNED with function call below
      ...
      THEN
         ...
         IF P_oc_LATEST_VERSION_IND IS NULL
         THEN
            v_oc_ind.latest_version_ind := FALSE;
         ELSE
            v_oc_rec.latest_version_ind := P_oc_LATEST_VERSION_IND;
         ...
         IF P_OC_DEFINITION_SOURCE IS NULL
         THEN
   ...
   PROCEDURE SET_OC_CONDR (P_OC_con_array              IN     VARCHAR2,
   ...
   BEGIN
      ...
      v_condr_idseq :=
         sbrext_common_routines.SET_DERIVATION_RULE (p_oc_con_array);
      ...
      SELECT COUNT (*)
        INTO v_exists
        FROM OBJECT_CLASSES_EXT
       WHERE conte_idseq = v_conte_idseq AND condr_idseq = v_condr_idseq;
   ...
   /* GF32649 not called from Java but from PL/SQL codes */
   PROCEDURE SET_PROP_CONDR (P_PROP_con_array              IN     VARCHAR2,
   ...
   BEGIN
      ...
      v_condr_idseq :=
         sbrext_common_routines.SET_DERIVATION_RULE (p_prop_con_array);
   ...
   /* GF32649 not called from Java but from PL/SQL codes */
   PROCEDURE SET_CONCEPT (P_RETURN_CODE                   OUT VARCHAR2,
                          P_ACTION                     IN     VARCHAR2,
   ...
   BEGIN
      ...
      IF P_ACTION = 'DEL'
      THEN                                          --we are deleting a record
         IF P_CON_IDSEQ IS NULL
         THEN
            P_RETURN_CODE := 'API_CON_400';   --for deleted the ID idmandatory
            RETURN;
         ELSE
            IF NOT Sbrext_Common_Routines.ac_exists (P_CON_IDSEQ)
            THEN
      ...
      THEN                                          --we are updating a record
         ...
         IF     (   P_CON_PREFERRED_NAME IS NOT NULL
                 OR P_CON_CONTE_IDSEQ IS NOT NULL)
            AND v_asl_name = 'RELEASED'
         THEN
      ...
      IF P_CON_LATEST_VERSION_IND IS NOT NULL
      THEN
         IF P_CON_LATEST_VERSION_IND NOT IN ('Yes', 'No')
         THEN
      ...
      --Check to see that all VARCHAR2 and  VARCHAR2 parameters have correct length
      IF LENGTH (P_CON_PREFERRED_NAME) >
            Sbrext_Column_Lengths.L_CON_PREFERRED_NAME
      THEN
         P_RETURN_CODE := 'API_CON_111'; --Length of preferred_name exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_CON_PREFERRED_DEFINITION) >
            Sbrext_Column_Lengths.L_CON_PREFERRED_DEFINITION
      THEN
         P_RETURN_CODE := 'API_CON_113'; --Length of Preferred_definition exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_CON_LONG_NAME) > Sbrext_Column_Lengths.L_CON_LONG_NAME
      THEN
         P_RETURN_CODE := 'API_CON_114'; --Length of Long_name exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_CON_ASL_NAME) > Sbrext_Column_Lengths.L_CON_ASL_NAME
      THEN
         P_RETURN_CODE := 'API_CON_115'; --Length of asl_name exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_CON_CHANGE_NOTE) > Sbrext_Column_Lengths.L_CON_CHANGE_NOTE
      THEN
         P_RETURN_CODE := 'API_CON_128'; --Length of change_note exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_CON_ORIGIN) > sbrext_column_lengths.L_CON_ORIGIN
      THEN
         P_RETURN_CODE := 'API_CON_129'; --Length of origin exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_CON_DEFINITION_SOURCE) >
            sbrext_column_lengths.L_CON_ORIGIN
      THEN
         P_RETURN_CODE := 'API_CON_141'; --Length of definition_source exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_CON_EVS_SOURCE) > sbrext_column_lengths.L_CON_EVS_SOURCE
      THEN
      ...
      --check to see that charachter strings are valid
      IF NOT Sbrext_Common_Routines.valid_alphanumeric (P_CON_PREFERRED_NAME)
      THEN
         P_RETURN_CODE := 'API_CON_130'; -- Data Element Preferred Name has invalid Ccharacters
         RETURN;
      END IF;

      IF NOT Sbrext_Common_Routines.valid_char (P_CON_PREFERRED_DEFINITION)
      THEN
         P_RETURN_CODE := 'API_CON_133'; -- Data Element Preferred Definition has invalid characters
         RETURN;
      END IF;

      IF NOT Sbrext_Common_Routines.valid_char (P_CON_LONG_NAME)
      THEN
      ...
      --check to see that Context, Workflow Status, Data Element Concept, Value Domain already exist in the database
      IF P_CON_CONTE_IDSEQ IS NOT NULL
      THEN
         IF NOT Sbrext_Common_Routines.context_exists (P_CON_CONTE_IDSEQ)
         THEN
      ...
      IF P_CON_ASL_NAME IS NOT NULL
      THEN
         IF NOT Sbrext_Common_Routines.workflow_exists (P_CON_ASL_NAME)
         THEN
      ...
      --check to see that begin data and end date are valid dates
      IF (P_CON_BEGIN_DATE IS NOT NULL)
      THEN
         Sbrext_Common_Routines.valid_date (P_RETURN_CODE,
                                            P_CON_BEGIN_DATE,
                                            v_begin_date);
      ...
      IF (P_CON_BEGIN_DATE IS NOT NULL AND P_CON_END_DATE IS NOT NULL)
      THEN
         IF (v_end_date < v_begin_date)
         THEN
            ...
            RETURN;
         END IF;
      ELSIF (P_CON_END_DATE IS NOT NULL AND P_CON_BEGIN_DATE IS NULL)
      THEN
      ...
      THEN
         ...
         IF P_CON_VERSION IS NULL
         THEN
            P_CON_VERSION := 1;
         END IF;

         IF P_CON_ASL_NAME IS NULL
         THEN
            -- 19-Mar-2004, W. Ver Hoef - substituted UNASSIGNED with function call below
      ...
      THEN
         ...
         IF P_CON_LATEST_VERSION_IND IS NULL
         THEN
            v_con_ind.latest_version_ind := FALSE;
         ELSE
            v_con_rec.latest_version_ind := P_CON_LATEST_VERSION_IND;
         ...
         IF P_CON_DEFINITION_SOURCE IS NULL
         THEN
   ...
   BEGIN
      ...
      IF P_ACTION = 'DEL'
      THEN                                          --we are deleting a record
         IF P_REP_IDSEQ IS NULL
         THEN
            P_RETURN_CODE := 'API_REP_400'; --for deleting the ID is mandatory
            RETURN;
         ELSE
            IF NOT Sbrext_Common_Routines.ac_exists (P_REP_IDSEQ)
            THEN
      ...
      THEN                                          --we are updating a record
         ...
         IF     (   P_REP_PREFERRED_NAME IS NOT NULL
                 OR P_REP_CONTE_IDSEQ IS NOT NULL)
            AND v_Asl_name = 'RELEASED'
         THEN
      ...
      IF P_REP_LATEST_VERSION_IND IS NOT NULL
      THEN
         IF P_REP_LATEST_VERSION_IND NOT IN ('Yes', 'No')
         THEN
      ...
      --Check to see that all VARCHAR2 and VARCHAR2 parameters have correct length
      IF LENGTH (P_REP_PREFERRED_NAME) >
            Sbrext_Column_Lengths.L_REP_PREFERRED_NAME
      THEN
         P_RETURN_CODE := 'API_REP_111'; --Length of preferred_name exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_REP_PREFERRED_DEFINITION) >
            Sbrext_Column_Lengths.L_REP_PREFERRED_DEFINITION
      THEN
         P_RETURN_CODE := 'API_REP_113'; --Length of Preferred_definition exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_REP_LONG_NAME) > Sbrext_Column_Lengths.L_REP_LONG_NAME
      THEN
         P_RETURN_CODE := 'API_REP_114'; --Length of Long_name exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_REP_ASL_NAME) > Sbrext_Column_Lengths.L_REP_ASL_NAME
      THEN
         P_RETURN_CODE := 'API_REP_115'; --Length of asl_name exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_REP_CHANGE_NOTE) > Sbrext_Column_Lengths.L_REP_CHANGE_NOTE
      THEN
      ...
      --check to see that charachter strings are valid
      IF NOT Sbrext_Common_Routines.valid_alphanumeric (P_REP_PREFERRED_NAME)
      THEN
         P_RETURN_CODE := 'API_REP_130'; -- Data Element Preferred Name has invalid Ccharacters
         RETURN;
      END IF;

      IF NOT Sbrext_Common_Routines.valid_char (P_REP_PREFERRED_DEFINITION)
      THEN
         P_RETURN_CODE := 'API_REP_133'; -- Data Element Preferred Definition has invalid characters
         RETURN;
      END IF;

      IF NOT Sbrext_Common_Routines.valid_char (P_REP_LONG_NAME)
      THEN
      ...
      --check to see that Context, Workflow Status, Data Element Concept, Value Domain already exist in the database
      IF P_REP_CONTE_IDSEQ IS NOT NULL
      THEN
         IF NOT Sbrext_Common_Routines.context_exists (P_REP_CONTE_IDSEQ)
         THEN
      ...
      IF P_REP_ASL_NAME IS NOT NULL
      THEN
         IF NOT Sbrext_Common_Routines.workflow_exists (P_REP_ASL_NAME)
         THEN
      ...
      --check to see that begin data and end date are valid dates
      IF (P_REP_BEGIN_DATE IS NOT NULL)
      THEN
         Sbrext_Common_Routines.valid_date (P_RETURN_CODE,
                                            P_REP_BEGIN_DATE,
                                            v_begin_date);
      ...
      IF (P_REP_BEGIN_DATE IS NOT NULL AND P_REP_END_DATE IS NOT NULL)
      THEN
         IF (v_end_date < v_begin_date)
         THEN
            ...
            RETURN;
         END IF;
      ELSIF (P_REP_END_DATE IS NOT NULL AND P_REP_BEGIN_DATE IS NULL)
      THEN
      ...
      THEN
         ...
         IF P_REP_VERSION IS NULL
         THEN
            P_REP_VERSION := 1;
         END IF;

         IF P_REP_ASL_NAME IS NULL
         THEN
            -- 19-Mar-2004, W. Ver Hoef - substituted UNASSIGNED with function call below
      ...
      THEN
         ...
         IF P_REP_LATEST_VERSION_IND IS NULL
         THEN
            v_rep_ind.latest_version_ind := FALSE;
         ELSE
            v_rep_rec.latest_version_ind := P_REP_LATEST_VERSION_IND;
         ...
         IF P_REP_DEFINITION_SOURCE IS NULL
         THEN
   ...
   /* GF32649 not called from Java but from PL/SQL codes */
   PROCEDURE SET_REP_CONDR (P_REP_con_array              IN     VARCHAR2,
   ...
   BEGIN
      ...
      v_condr_idseq :=
         sbrext_common_routines.SET_DERIVATION_RULE (p_rep_con_array);
   ...
   PROCEDURE SET_QUAL (P_RETURN_CODE              OUT VARCHAR2,
                       P_ACTION                IN     VARCHAR2,
   ...
   BEGIN
      ...
      THEN                                         --we are inserting a record
         --Check to see that all mandatory parameters are either not null
         IF P_QUAL_qualifier_name IS NULL
         THEN
      ...
      THEN
         ...
         IF NOT Sbrext_Common_Routines.QUAL_exists (P_QUAL_qualifier_name)
         THEN
      ...
      --Check to see that all VARCHAR2 and  VARCHAR2 parameters have correct length
      IF LENGTH (P_QUAL_qualifier_name) >
            Sbrext_Column_Lengths.L_QUAL_qualifier_name
      THEN
         P_RETURN_CODE := 'API_QUAL_111'; --Length of qualifier_name exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_QUAL_DESCRIPTION) >
            Sbrext_Column_Lengths.L_QUAL_DESCRIPTION
      THEN
         P_RETURN_CODE := 'API_QUAL_113'; --Length of DESCRIPTION exceeds maximum length
         RETURN;
      END IF;

      IF LENGTH (P_QUAL_COMMENTS) > Sbrext_Column_Lengths.L_QUAL_COMMENTS
      THEN
      ...
      --check to see that charachter strings are valid
      IF NOT Sbrext_Common_Routines.valid_char (P_QUAL_qualifier_name)
      THEN
         P_RETURN_CODE := 'API_QUAL_130'; -- Value Meaning Short Meaning has invalid characters
         RETURN;
      END IF;

      IF NOT Sbrext_Common_Routines.valid_char (P_QUAL_DESCRIPTION)
      THEN
         P_RETURN_CODE := 'API_QUAL_133'; -- Value Meaning Description has invalid characters
         RETURN;
      END IF;

      IF NOT Sbrext_Common_Routines.valid_char (P_QUAL_COMMENTS)
      THEN
         P_RETURN_CODE := 'API_QUAL_134'; -- Value Meaning Comment has invalid characters
         RETURN;
      END IF;

      IF P_QUAL_CON_IDSEQ IS NOT NULL
      THEN
         IF NOT Sbrext_Common_Routines.AC_EXISTS (p_qual_con_idseq)
         THEN
      ...
      THEN
         --check to see that Short Name Does not already Exist
         IF Sbrext_Common_Routines.QUAL_exists (P_QUAL_qualifier_name)
         THEN
   ...
   PROCEDURE set_cd (p_action                    IN     VARCHAR2,
   ...
   BEGIN
      ...
      IF p_action = 'UPD'
      THEN
         IF p_cd_idseq IS NULL
         THEN
            p_return_code := 'API_CD_105';           -- PK cannot be null here
            RETURN;
         END IF;

         IF NOT Sbrext_Common_Routines.ac_exists (p_cd_idseq)
         THEN
      ...
      --Check to see that all VARCHAR2 and CHAR parameters have correct length
      IF LENGTH (p_cd_idseq) > Sbrext_Column_Lengths.l_cd_cd_idseq
      THEN
         p_return_code := 'API_CD_106';
         RETURN;
      END IF;

      IF LENGTH (p_cd_preferred_name) >
            Sbrext_Column_Lengths.l_cd_preferred_name
      THEN
         p_return_code := 'API_CD_107';
         RETURN;
      END IF;

      IF LENGTH (p_cd_long_name) > Sbrext_Column_Lengths.l_cd_long_name
      THEN
         p_return_code := 'API_CD_108';
         RETURN;
      END IF;

      IF LENGTH (p_cd_asl_name) > Sbrext_Column_Lengths.l_cd_asl_name
      THEN
         p_return_code := 'API_CD_109';
         RETURN;
      END IF;

      IF LENGTH (p_cd_conte_idseq) > Sbrext_Column_Lengths.l_cd_conte_idseq
      THEN
         p_return_code := 'API_CD_110';
         RETURN;
      END IF;

      IF LENGTH (p_cd_preferred_definition) >
            Sbrext_Column_Lengths.l_cd_preferred_definition
      THEN
         p_return_code := 'API_CD_111';
         RETURN;
      END IF;

      IF LENGTH (p_cd_dimensionality) >
            Sbrext_Column_Lengths.l_cd_dimensionality
      THEN
         p_return_code := 'API_CD_112';
         RETURN;
      END IF;

      IF LENGTH (p_cd_latest_version_ind) >
            Sbrext_Column_Lengths.l_cd_latest_version_ind
      THEN
         p_return_code := 'API_CD_113';
         RETURN;
      END IF;

      IF LENGTH (p_cd_change_note) > Sbrext_Column_Lengths.l_cd_change_note
      THEN
         p_return_code := 'API_CD_114';
         RETURN;
      END IF;

      IF LENGTH (p_cd_origin) > Sbrext_Column_Lengths.l_cd_origin
      THEN
      ...
      IF NOT Sbrext_Common_Routines.valid_char (p_cd_long_name)
      THEN
         p_return_code := 'API_CD_116';
         RETURN;
      END IF;

      IF NOT Sbrext_Common_Routines.valid_char (p_cd_preferred_definition)
      THEN
         p_return_code := 'API_CD_117';
         RETURN;
      END IF;

      IF NOT Sbrext_Common_Routines.valid_char (p_cd_change_note)
      THEN
         p_return_code := 'API_CD_118';
         RETURN;
      END IF;

      IF p_cd_conte_idseq IS NOT NULL
      THEN
         IF NOT Sbrext_Common_Routines.context_exists (p_cd_conte_idseq)
         THEN
      ...
      IF p_cd_asl_name IS NOT NULL
      THEN
         IF NOT Sbrext_Common_Routines.workflow_exists (p_cd_asl_name)
         THEN
      ...
      THEN
         ...
         IF Sbrext_Common_Routines.cd_exists (p_cd_preferred_name,
                                              p_cd_version,
                                              p_cd_conte_idseq)
         THEN
      ...
      THEN
         ...
         IF p_cd_dimensionality IS NULL
         THEN
         ...
         IF p_cd_latest_version_ind IS NULL
         THEN
   ...
   PROCEDURE set_complex_de (p_action              IN     VARCHAR2,
   ...
   BEGIN
      ...
      IF p_action NOT IN ('INS', 'UPD', 'DEL')
      THEN                      -- 23-Mar-2004, W. Ver Hoef - added DEL option
      ...
      THEN
         ...
         IF p_cdt_p_de_idseq IS NULL
         THEN
            p_return_code := 'API_CDE_101';  -- parameter value cannot be null
            RETURN;
         END IF;

         IF p_cdt_crtl_name IS NULL
         THEN
            p_return_code := 'API_CDT_102';  -- parameter value cannot be null
            RETURN;
         END IF;

         IF NOT Sbrext_Common_Routines.ac_exists (p_cdt_p_de_idseq)
         THEN
            p_return_code := 'API_CDT_103';                    -- DE not found
            RETURN;
         END IF;

         IF NOT Sbrext_Common_Routines.crtl_exists (p_cdt_crtl_name)
         THEN
      ...
      THEN
         ...
         IF NOT Sbrext_Common_Routines.cdt_exists (p_cdt_p_de_idseq)
         THEN
         ...
         IF     p_cdt_crtl_name IS NOT NULL
            AND NOT Sbrext_Common_Routines.crtl_exists (p_cdt_crtl_name)
         THEN
            p_return_code := 'API_CDT_104';      -- Complex Rep Type not found
            RETURN;
         END IF;
      ...
      --Check to see that all VARCHAR2 and CHAR parameters have correct length
      IF LENGTH (p_cdt_concat_char) > Sbrext_Column_Lengths.l_cdt_concat_char
      THEN
         p_return_code := 'API_CDT_105';
         RETURN;
      END IF;

      IF LENGTH (p_cdt_methods) > Sbrext_Column_Lengths.l_cdt_methods
      THEN
         p_return_code := 'API_CDT_106';
         RETURN;
      END IF;

      IF LENGTH (p_cdt_rule) > Sbrext_Column_Lengths.l_cdt_rule
      THEN
      ...
      --check to see that character strings are valid
      IF NOT Sbrext_Common_Routines.valid_char (p_cdt_methods)
      THEN
         p_return_code := 'API_CDT_110';
         RETURN;
      END IF;

      IF NOT Sbrext_Common_Routines.valid_char (p_cdt_rule)
      THEN
      ...
      THEN
         --check to see that UK components do not already exist
         IF Sbrext_Common_Routines.cdt_exists (p_cdt_p_de_idseq)
         THEN
   ...
   PROCEDURE set_cde_relationship (p_action              IN     VARCHAR2,
   ...
   BEGIN
      ...
      IF p_action NOT IN ('INS', 'UPD', 'DEL')
      THEN            -- 23-Mar-2004, W. Ver Hoef - added DEL per SPRF_2.1_06a
      ...
      IF p_action = 'UPD'
      THEN
         IF p_cdr_idseq IS NULL
         THEN
            p_return_code := 'API_CDR_103';          -- PK cannot be null here
            RETURN;
         END IF;

         IF NOT Sbrext_Common_Routines.cdr_exists (p_cdr_idseq)
         THEN
      ...
      IF LENGTH (p_cdr_display_order) > 4
      THEN
      ...
      IF p_cdr_display_order IS NOT NULL
      THEN
         IF NOT (    ASCII (SUBSTR (p_cdr_display_order, 1, 1)) BETWEEN 48
                                                                    AND 57
                 AND ASCII (NVL (SUBSTR (p_cdr_display_order, 2, 1), '0')) BETWEEN 48
                                                                               AND 57
                 AND ASCII (NVL (SUBSTR (p_cdr_display_order, 3, 1), '0')) BETWEEN 48
                                                                               AND 57
                 AND ASCII (NVL (SUBSTR (p_cdr_display_order, 4, 1), '0')) BETWEEN 48
                                                                               AND 57)
         THEN
      ...
      IF p_cdr_p_de_idseq IS NOT NULL
      THEN
         IF NOT Sbrext_Common_Routines.cdt_exists (p_cdr_p_de_idseq)
         THEN
      ...
      IF p_cdr_c_de_idseq IS NOT NULL
      THEN
         IF NOT Sbrext_Common_Routines.ac_exists (p_cdr_c_de_idseq)
         THEN
      ...
      THEN
         ...
         IF p_cdr_display_order IS NULL
         THEN
   ...
   PROCEDURE set_registration (p_action                   IN     VARCHAR2,
   ...
   BEGIN
      ...
      IF p_action NOT IN ('INS', 'UPD', 'DEL')
      THEN             -- 23-Mar-2004, W. Ver Hoef - added DEL per SPRF_2.1_03
      ...
      THEN
         --Check to see that all mandatory parameters are not null
         IF p_ar_ac_idseq IS NULL
         THEN
            p_return_code := 'API_AR_101';   -- parameter value cannot be null
            RETURN;
         END IF;

         IF p_ar_registration_status IS NULL
         THEN
      ...
      IF p_action = 'UPD'
      THEN
         IF p_ar_idseq IS NULL
         THEN
            p_return_code := 'API_AR_103';           -- PK cannot be null here
            RETURN;
         END IF;

         IF NOT Sbrext_Common_Routines.ar_exists (p_ar_idseq)
         THEN
      ...
      IF LENGTH (p_ar_registration_status) >
            Sbrext_Column_Lengths.l_ar_registration_status
      THEN
         p_return_code := 'API_AR_105';
         RETURN;
      END IF;

      IF p_ar_ac_idseq IS NOT NULL
      THEN
         IF NOT Sbrext_Common_Routines.ac_exists (p_ar_ac_idseq)
         THEN
      ...
      IF p_ar_registration_status IS NOT NULL
      THEN
         IF NOT Sbrext_Common_Routines.rsl_exists (p_ar_registration_status)
         THEN
   ...
   PROCEDURE set_contact_comm (p_return_code        OUT VARCHAR2,
                               p_action          IN     VARCHAR2,
   ...
   BEGIN
      ...
      THEN                                                   -- insert section
         ...
         IF (   p_ctl_name IS NULL
             OR p_rank_order IS NULL
             OR p_cyber_address IS NULL)
         THEN
            ...
            IF p_rank_order IS NULL
            THEN
               p_return_code := p_return_code || ' p_rank_order ';
            END IF;

            IF p_cyber_address IS NULL
            THEN
         ...
         -- Validate ctl_name
         IF NOT sbrext_common_routines.comm_type_exists (p_ctl_name)
         THEN
         ...
         IF p_per_idseq IS NOT NULL
         THEN
            IF NOT sbrext_common_routines.per_exists (p_per_idseq)
            THEN
         ...
         IF p_org_idseq IS NOT NULL
         THEN
            IF NOT sbrext_common_routines.org_exists (p_org_idseq)
            THEN
         ...
         IF LENGTH (p_ctl_name) >
               sbrext_column_lengths.get_vc2_col_length (
                  p_owner   => 'SBR',
         ...
         IF LENGTH (p_cyber_address) >
               sbrext_column_lengths.get_vc2_col_length (
                  p_owner   => 'SBR',
                  p_tname   => 'CONTACT_COMMS',
                  p_cname   => 'CYBER_ADDRESS')
         THEN
         ...
         THEN
            ...
            THEN
               p_return_code :=
                  'API_ACC_553: Arc violation [org_idseq + per_idseq]';
            END IF;
         END IF;

         IF (p_per_idseq IS NULL AND p_org_idseq IS NULL)
         THEN
         ...
         -- set row
         INSERT INTO contact_comms_view (ccomm_idseq,
                                         org_idseq,
                                         per_idseq,
                                         ctl_name,
                                         rank_order,
      ...
      THEN                                                   -- update section
         ...
         -- mandatory columns
         IF p_ctl_name IS NOT NULL
         THEN
            r_con_comm.ctl_name := p_ctl_name;
         END IF;

         IF p_rank_order IS NOT NULL
         THEN
            r_con_comm.rank_order := p_rank_order;
         END IF;

         IF p_cyber_address IS NOT NULL
         THEN
   ...
   PROCEDURE set_contact_addr (p_return_code        OUT VARCHAR2,
                               p_action          IN     VARCHAR2,
   ...
   BEGIN
      ...
      THEN                                                   -- insert section
         -- check for mandatory columns
         IF (   p_atl_name IS NULL
             OR p_rank_order IS NULL
             OR p_addr_line1 IS NULL
             OR p_city IS NULL
             OR p_state_prov IS NULL
         ...
         -- Validate atl_name
         IF NOT sbrext_common_routines.addr_type_exists (p_atl_name)
         THEN
         ...
         IF p_addr_line2 IS NOT NULL
         THEN
            IF LENGTH (p_addr_line2) >
                  sbrext_column_lengths.get_vc2_col_length (
                     p_owner   => 'SBR',
                     p_tname   => 'CONTACT_ADDRESSES',
                     p_cname   => 'ADDR_LINE2')
         ...
         -- set row
         INSERT INTO contact_addresses_view (caddr_idseq,
                                             org_idseq,
                                             per_idseq,
                                             atl_name,
                                             rank_order,
   ...
   */
   ...
   PROCEDURE SET_PV (P_UA_NAME                  IN     VARCHAR2,
                     P_RETURN_CODE                 OUT VARCHAR2,
                     P_ACTION                   IN     VARCHAR2,
                     P_PV_PV_IDSEQ              IN OUT VARCHAR2,
                     P_PV_VALUE                 IN OUT VARCHAR2,
   ...
   BEGIN
      DBMS_OUTPUT.put_line (
         'SET_VD_PVS entered P_VDPVS_PV_IDSEQ = [' || P_VDPVS_PV_IDSEQ || ']');
      DBMS_OUTPUT.put_line (
            'SET_VD_PVS entered P_VDPVS_CONTE_IDSEQ = ['
         || P_VDPVS_CONTE_IDSEQ
         || ']');
     ...
     RETURN;
      ...
      THEN
         ...
         BEGIN
            DBMS_OUTPUT.put_line ('JR1074 SBREXT_SET_ROW jt200 inserting');
            cg$vd_pvs_view.ins (v_vp_rec, v_vp_ind);
            DBMS_OUTPUT.put_line ('JR1074 SBREXT_SET_ROW jt200 inserted');
         EXCEPTION
            WHEN OTHERS
            THEN
               P_RETURN_CODE := 'API_VDPVS_500';      --Error inserting VD_PVS
               DBMS_OUTPUT.put_line ('JR1074 API_VDPVS_500 insertion error');
               DBMS_OUTPUT.put_line ('JR1074 err3.11: ' || SQLERRM);
         END;
      ELSIF (P_ACTION = 'UPD')
      THEN                                   --  Added 8/23/2004 sprf_2.1.1_2b
         admin_security_util.seteffectiveuser (p_ua_name);
   ...
   PROCEDURE SET_RD (P_UA_NAME            IN     VARCHAR2,
                     P_RETURN_CODE           OUT VARCHAR2,
                     P_ACTION             IN     VARCHAR2,
                     P_RD_RD_IDSEQ        IN OUT VARCHAR2,
                     P_RD_NAME            IN OUT VARCHAR2,
   ...
   PROCEDURE SET_DES (P_UA_NAME             IN     VARCHAR2,
                      P_RETURN_CODE            OUT VARCHAR2,
                      P_ACTION              IN     VARCHAR2,
                      P_DES_DESIG_IDSEQ     IN OUT VARCHAR2,
                      P_DES_NAME            IN OUT VARCHAR2,
   ...
   PROCEDURE SET_CSCSI (P_UA_NAME                   IN     VARCHAR2,
                        P_RETURN_CODE                  OUT VARCHAR2,
                        P_ACTION                    IN     VARCHAR2,
                        P_CSCSI_CS_CSI_IDSEQ        IN OUT VARCHAR2,
                        P_CSCSI_CS_IDSEQ            IN OUT VARCHAR2,
   ...
   PROCEDURE SET_ACCSI (P_UA_NAME               IN     VARCHAR2,
                        P_RETURN_CODE              OUT VARCHAR2,
                        P_ACTION                IN     VARCHAR2,
                        P_ACCSI_AC_CSI_IDSEQ    IN OUT VARCHAR2,
                        P_ACCSI_AC_IDSEQ        IN OUT VARCHAR2,
   ...
   PROCEDURE SET_SRC (P_UA_NAME             IN     VARCHAR2,
                      P_RETURN_CODE            OUT VARCHAR2,
                      P_ACTION              IN     VARCHAR2,
                      P_SRC_SRC_NAME        IN OUT VARCHAR2,
                      P_SRC_DESCRIPTION     IN OUT VARCHAR2,
   ...
   PROCEDURE SET_TS (P_UA_NAME            IN     VARCHAR2,
                     P_RETURN_CODE           OUT VARCHAR2,
                     P_ACTION             IN     VARCHAR2,
                     P_TS_TS_IDSEQ        IN OUT VARCHAR2,
                     P_TS_QC_IDSEQ        IN OUT VARCHAR2,
   ...
   PROCEDURE SET_ACSRC (P_UA_NAME                IN     VARCHAR2,
                        P_RETURN_CODE               OUT VARCHAR2,
                        P_ACTION                 IN     VARCHAR2,
                        P_ACSRC_ACS_IDSEQ        IN OUT VARCHAR2,
                        P_ACSRC_AC_IDSEQ         IN OUT VARCHAR2,
   ...
   PROCEDURE SET_VPSRC (P_UA_NAME                IN     VARCHAR2,
                        P_RETURN_CODE               OUT VARCHAR2,
                        P_ACTION                 IN     VARCHAR2,
                        P_VPSRC_VPS_IDSEQ        IN OUT VARCHAR2,
                        P_VPSRC_VP_IDSEQ         IN OUT VARCHAR2,
   ...
   PROCEDURE SET_REL (P_UA_NAME             IN     VARCHAR2,
                      P_RETURN_CODE            OUT VARCHAR2,
                      P_ACTION              IN     VARCHAR2,
                      P_REL_TABLE           IN     VARCHAR2,
                      P_REL_REL_IDSEQ       IN OUT VARCHAR2,
   ...
   PROCEDURE SET_CD_VMS (P_UA_NAME               IN     VARCHAR2,
                         P_RETURN_CODE              OUT VARCHAR2,
                         P_ACTION                IN     VARCHAR2,
                         P_CDVMS_CV_IDSEQ        IN OUT VARCHAR2,
                         P_CDVMS_CD_IDSEQ        IN OUT VARCHAR2,
   ...
   PROCEDURE SET_PROPERTY (P_UA_NAME                     IN     VARCHAR2,
                           P_RETURN_CODE                    OUT VARCHAR2,
                           P_ACTION                      IN     VARCHAR2,
                           P_PROP_IDSEQ                  IN OUT VARCHAR2,
                           P_PROP_PREFERRED_NAME         IN OUT VARCHAR2,
   ...
   PROCEDURE SET_OBJECT_CLASS (P_UA_NAME                   IN     VARCHAR2,
                               P_RETURN_CODE                  OUT VARCHAR2,
                               P_ACTION                    IN     VARCHAR2,
                               P_oc_IDSEQ                  IN OUT VARCHAR2,
                               P_oc_PREFERRED_NAME         IN OUT VARCHAR2,
   ...
   PROCEDURE SET_OC_CONDR (P_UA_NAME                   IN     VARCHAR2,
   ...
   PROCEDURE SET_PROP_CONDR (P_UA_NAME                     IN     VARCHAR2,
   ...
   PROCEDURE SET_CONCEPT (P_UA_NAME                    IN     VARCHAR2,
                          P_RETURN_CODE                   OUT VARCHAR2,
                          P_ACTION                     IN     VARCHAR2,
                          P_CON_IDSEQ                     OUT VARCHAR2,
                          P_CON_PREFERRED_NAME         IN OUT VARCHAR2,
   ...
   BEGIN
      ...
      DBMS_OUTPUT.put_line (
            'SET_CONCEPT P_DEFAULT_CONTE_NAME = ['
         || P_DEFAULT_CONTE_NAME
         || ']');
   ...
   PROCEDURE SET_REP_CONDR (P_UA_NAME                    IN     VARCHAR2,
   ...
   PROCEDURE SET_QUAL (P_UA_NAME               IN     VARCHAR2,
                       P_RETURN_CODE              OUT VARCHAR2,
                       P_ACTION                IN     VARCHAR2,
                       P_QUAL_qualifier_name   IN OUT VARCHAR2,
                       P_QUAL_DESCRIPTION      IN OUT VARCHAR2,
   ...
   PROCEDURE set_cd (P_UA_NAME                   IN     VARCHAR2,
                     p_action                    IN     VARCHAR2,
                     p_cd_idseq                  IN OUT VARCHAR2,
                     p_cd_version                IN OUT VARCHAR2,
                     p_cd_preferred_name         IN OUT VARCHAR2,
   ...
   PROCEDURE set_complex_de (P_UA_NAME             IN     VARCHAR2,
                             p_action              IN     VARCHAR2,
                             p_cdt_p_de_idseq      IN     VARCHAR2,
                             p_cdt_methods         IN     VARCHAR2,
                             p_cdt_rule            IN     VARCHAR2,
   ...
   PROCEDURE set_cde_relationship (P_UA_NAME             IN     VARCHAR2,
                                   p_action              IN     VARCHAR2,
                                   p_cdr_idseq           IN OUT VARCHAR2,
                                   p_cdr_p_de_idseq      IN OUT VARCHAR2,
                                   p_cdr_c_de_idseq      IN OUT VARCHAR2,
   ...
   PROCEDURE set_registration (P_UA_NAME                  IN     VARCHAR2,
                               p_action                   IN     VARCHAR2,
                               p_ar_idseq                 IN OUT VARCHAR2,
                               p_ar_ac_idseq              IN OUT VARCHAR2,
                               p_ar_registration_status   IN OUT VARCHAR2,
   ...
   -- SPRF_3.1_16i (TT#1001)
   PROCEDURE set_contact_comm (P_UA_NAME         IN     VARCHAR2,
                               p_return_code        OUT VARCHAR2,
                               p_action          IN     VARCHAR2,
                               p_ccomm_idseq     IN OUT VARCHAR2,
                               p_org_idseq       IN OUT VARCHAR2,
   ...
   -- SPRF_3.1_16j (TT#1001)
   PROCEDURE set_contact_addr (P_UA_NAME         IN     VARCHAR2,
                               p_return_code        OUT VARCHAR2,
                               p_action          IN     VARCHAR2,
                               p_caddr_idseq     IN OUT VARCHAR2,
                               p_org_idseq       IN OUT VARCHAR2,
   ...
   PROCEDURE set_ac_contact (P_UA_NAME         IN     VARCHAR2,
                             p_return_code        OUT VARCHAR2,
                             p_action          IN     VARCHAR2,
   ...
   BEGIN
      ...
      THEN                                                   -- insert section
         ...
         IF p_ac_idseq IS NOT NULL
         THEN
            IF NOT sbrext_common_routines.ac_exists (p_ac_idseq)
            THEN
         ...
         IF p_cs_csi_idseq IS NOT NULL
         THEN
            IF NOT sbrext_common_routines.org_exists (p_cs_csi_idseq)
            THEN
         ...
         IF p_contact_role IS NOT NULL
         THEN
            IF NOT sbrext_common_routines.contact_role_exists (
                      p_contact_role)
            THEN
         ...
         THEN
            ...
            THEN
               p_return_code :=
                  'API_ACC_550: Arc violation [ac_idseq + ar_idseq]';
            ELSIF p_cs_csi_idseq IS NOT NULL
            THEN
               p_return_code :=
                  'API_ACC_551: Arc violation [ac_idseq + cs_csi_idseq]';
            END IF;
         END IF;
         ...
         THEN
            ...
            THEN
               p_return_code :=
                  'API_ACC_552:Arc violation [cs_csi_idseq + ar_idseq]';
            END IF;
         END IF;

         IF (    p_ar_idseq IS NULL
             AND p_cs_csi_idseq IS NULL
             AND p_ac_idseq IS NULL)
         THEN
            p_return_code :=
               'API_ACC_560:  AC-AR-CS_CSI Arc violation [no values]';
         END IF;
         ...
         -- set row
         INSERT INTO ac_contacts_view (acc_idseq,
                                       org_idseq,
                                       per_idseq,
                                       ac_idseq,
                                       rank_order,
      ...
      THEN                                                   -- update section
         ...
         THEN
            p_return_code :=
               'API_ACC_560:  AC-CSI-CS_CSI Arc violation [no values]';
         END IF;
