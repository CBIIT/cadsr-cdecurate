...
public class EVSSearch implements Serializable
{
     ...
     * @throws Exception
     */
    public void fillOCVectors(
            String oc_condr_idseq, DEC_Bean m_DEC,
    ...
    public void fillPropVectors(
            String prop_condr_idseq, DEC_Bean m_DEC,
            String sMenu ) throws Exception
    {
        if( m_DEC != null )
        {
     ...
     */
    @SuppressWarnings( "unchecked" )
    public void fillRepVectors(
            String rep_condr_idseq, VD_Bean m_VD,
            String sMenu ) throws Exception
    {
        if( m_VD != null )
        {
...
//      }
     ...
     */
    @SuppressWarnings( "unchecked" )
    public HashMap<String, String> getSubConcepts(
            String dtsVocab, String conceptName,
...
//                      lbscm.setLexBIGService(evsService);
    ...
    @SuppressWarnings( "unchecked" )
    public String[] getAllSubConceptNames(
            String dtsVocab,
     ...
     */
    @SuppressWarnings( "unchecked" )
    public HashMap<String, String> getSuperConceptNamesImmediate(
            String dtsVocab,
     ...
     */
    @SuppressWarnings( "unchecked" )
    public HashMap<String, String> getSuperConceptNames(
            String dtsVocab, String conceptName,
            String conceptCode )
    {
     ...
     */
    public Vector<EVS_Bean> doVocabSearch(
            Vector<EVS_Bean> vConList,
            ...
            try
            {
                ...
                if( lstResults != null )
                {
                    ...
                    if( hType == null )
                        hType = new Hashtable();
                    skipConcept:
                    for( int i = 0; i < lstResults.getResolvedConceptReferenceCount(); i++ )
                    {
...
//                                                      }
            ...
            //begin GF32723
            else if( LexEVSHelper.isOtherVocabulary( dtsVocab ) )
    ...
    public Vector<EVS_Bean> doVocabSearchForNonNCItNonNCIm(
            Vector<EVS_Bean> vConList,
            String termStr, String dtsVocab, String sSearchIn,
            String namePropIn, String sSearchAC, String sIncludeRet,
            String sMetaSource, int iMetaLimit, boolean isMetaSearch,
...
//                                              }
    ...
    public String getDisplayName(
            String dtsVocab, ResolvedConceptReference dlc,
            String sName )
    {
    ...
    private ResolvedConceptReferenceList doConceptQuery(
            String vocabAccess, String termStr, //JT needs to look into this EVS search!!!
            ...
            else
            {
                if( vocabType.equals( "" ) || vocabType.equals( "NameType" ) ) // do concept name search
                    nodeSet = nodeSet.restrictToMatchingDesignations( termStr, //the text to match
                            ...
                            null ); //the language to match (null matches all)
                else if( vocabType.equals( "PropType" ) )
                { // do concept prop search
...
//                                              nodeSet = nodeSet.restrictToStatus(ActiveOption.ALL, null);
            ...
            //begin GF32723 just for troubleshooting, it does not fix/change anything
            if( lstResult != null
            ...
            //end GF32723 just for troubleshooting, it does not fix/change anything
        } catch( Exception ex )
        {
     ...
     * @return
     */
    private Vector<EVS_Bean> storeConceptToBean(
            Vector<EVS_Bean> vCons,
...
//        }
     ...
     * @return
     */
    private Vector<EVS_Bean> doMetaSearch(
            Vector<EVS_Bean> vList,
            String termStr, String sSearchIn, String sMetaSource,
            int iMetaLimit, String sVocab )
    {
...
//                                      ActiveOption.ALL, null).restrictToCodes(crefs).resolveToList(null, null, null, 1);
    ...
    private Vector<EVS_Bean> doMetaSearchForNonNCItNonNCIm(
            Vector<EVS_Bean> vList,
            String termStr, String sSearchIn, String sMetaSource,
                ...
                if( rcr != null )
                {
                    ...
                    if( definitions != null && definitions.length > 0 )
                    {
                        for( Definition defType : definitions )
                        {
                            ...
                            conBean.setEVSBean( sDefinition, sDefSource,
                                    sConName, sConName, sCodeType, sConID,
                                    Constants.DTS_VOCAB_NCIT,    //sVocab, 	//GF32723 last fix, sVocab needs to be NCIt!
...
//                    .append( " Text: " ).append( prop.getValue().getContent() ).toString() );
    ...
    public void doGetSuperConcepts() throws Exception
    {
        try
        {
            ...
            if( ( m_classReq.getParameter( "UISearchType" ) != null ) && ( !StringUtil.isHtmlAndScriptClean( m_classReq.getParameter( "UISearchType" ) ) ) )
            {
                logger.warn( "Bad value for UISearchType[" + m_classReq.getParameter( "UISearchType" ) + "]" );
     ...
     * @throws Exception
     */
    private void doTreeOpenToConcept(
            String actType, String searchType,
     ...
     * @throws Exception
     */
    public void doTreeSearchRequest(
            String sConceptName, String sConceptID,
            String strForward, String dtsVocab ) throws Exception
    {
        ...
        // CURATNTOOL-1107  The app scan only shows this one issue for this method, but others may still need to be fixed, so set a boolean here rather than throw Exception right here.
     ...
     * @param dtsVocab
     */
    private void doCallConceptSearch(
            String searchID, String sCCodeDB,
     ...
     * @param sCode
     */
    private void doCallTreeSearch(
            String actType, String dtsVocab,
    ...
    public EVS_Bean getThesaurusConceptForNonNCItNonNCIm( EVS_Bean eBean )
    {
        try
        {
            ...
            //GF32723
            if( dtsVocab != null && dtsVocab.equals( Constants.DTS_VOCAB_NCI_META ) )
            {    //e.g. "NCI MetaThesaurus"
                String previousDtsVocab = dtsVocab;
                dtsVocab = "MetaValue";
                logger.info( "==========$ EVSSearch:getThesaurusConcept() dtsVocab [" + previousDtsVocab + "] is replaced becoming [" + dtsVocab + "] $==========" );
            }
            ...
            if( dtsVocab != null && !dtsVocab.equals( nciVocab ) )
            {
                if( !dtsVocab.equals( EVSSearch.META_VALUE ) ) // "MetaValue"))
                {
                    ...
                    if( conType != null && !conType.equals( "" ) )
                    {
                        ...
                        {
                            //end GF32723
                            vList = this.doVocabSearchForNonNCItNonNCIm( vList, conID,
                    ...
                    if( conID == null || conID.equals( "" ) )
                    {
                        ...
                        conID = eBean.getCONCEPT_IDENTIFIER();
                        vList = new Vector<EVS_Bean>();
                        vList = this.doMetaSearchForNonNCItNonNCIm( vList, conID, "MetaCode",
                                vSource, 10, metaName );
            ...
            if( dtsVocab != null && !dtsVocab.equals( nciVocab ) )
            {
                if( !dtsVocab.equals( EVSSearch.META_VALUE ) ) // "MetaValue"))
                {
                    ...
                    if( conType != null && !conType.equals( "" ) )
                    {
                        ...
                        //begin GF32723
//                                                if(LexEVSHelper.isOtherVocabulary(dtsVocab)) {   //e.g. RadLex vocab for instance
//                                                       vList = this.doMetaSearchForNonNCItNonNCIm(vList, conID, "MetaCode", eDB, 10, metaName);
//                                                       logger.info("getThesaurusConcept:doMetaSearchForNonNCItNonNCIm() called for dtsVocab [" + dtsVocab + "] eDB [" + eDB + "] metaName [" + metaName + "]");
//                                                } else {
                        //end GF32723
                        ...
                        vList = this.doVocabSearch( vList, conID, nciVocab,
                                "Name", conType, "", "Exclude", "", 10, false,
                                -1, "", new HashSet<String>() );
                        logger.info( "getThesaurusConcept:doVocabSearch() called for dtsVocab [" + dtsVocab + "] eDB [" + eDB + "] metaName [" + metaName + "]" );
//                                                } //end GF32723

                        if( vList != null && vList.size() > 0 )
                        {
                            //GF32723 do not need to do it again!
//                                                        eBean = this.getNCIDefinition(vList, conID, conName); //get the right definition    //Should come here for GF32723
                            return eBean;
                        }
                        conID = "";
...
//                                                eBean = this.getNCIDefinition(vList, conID, conName); // (EVS_Bean)vList.elementAt(0);
     ...
     */
    private EVS_Bean getNCIPrefDefinition(
            Vector<EVS_Bean> selList,
        ...
        if( matches.getResolvedConceptReferenceCount() > 0 )
        {
            ...
            if( targetof != null )
            {
                ...
                for( int i = 0; i < associations.length; i++ )
                {
                    ...
                    { //blank screen due to NPE on superconcept
                ...
                if( sourceOf != null )
                {
                    ...
                    for( int i = 0; i < associations.length; i++ )
                    {
                        ...
                        { //blank screen due to NPE on superconcept
                            ...
                            {
                                AssociatedConcept ac = acl[j];
                                if( ac != null && ac.getCode() != null )
                                {    //JR1040 nothing to do with the ticket, just avoiding NPE
                                    if( resolveConcepts )
                                        ret.put( ac.getCode(), ac );
                                    else
                                    {
                                        if( ac.getEntityDescription() != null && ac.getEntityDescription().getContent() != null )
                                        {    //JR1040 nothing to do with the ticket, just avoiding NPE
                                            ret.put( ac.getCode(), ac.getEntityDescription().getContent() );
                                        }
                                    }
