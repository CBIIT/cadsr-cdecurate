...
//        	(new AltNamesDefsSessionHelper()).purgeAlternateListDEC(m_classReq);		//JR1016
        ...
        try
        {
            ...
            if( sCondr != null && !sCondr.equals( "" ) )
            {
                ...
                for( int i = 0; i < vCon.size(); i++ )
                {
                    ...
                    logger.debug( "At line 678 of DECServlet.java eBean= " + eBean );
                }
    ...
    public AC_Bean getACNames( EVS_Bean newBean, String nameAct, AC_Bean pageAC )
    {
        ...
        String sDef = "";
        //======================GF30798==============START
        String sComp = StringUtil.cleanJavascriptAndHtml( ( String ) m_classReq.getParameter( "sCompBlocks" ) );
            ...
            if( !sPrimary.equals( "" ) )
            {
                ...
                if( newBean == null )
                {
                    ...
                    if( !sDef.equals( "" ) )
                        sDef += "_"; // add definition
                    sDef += eCon.getPREFERRED_DEFINITION() + nvpValue;
                    logger.debug( "At line 833 of DECServlet.java sLongName: " + sLongName + ", sDef: " + sDef );
                }
                if( !sAbbName.equals( "" ) )
                    sAbbName += "_";
        ...
        for( int i = 0; i < vProperty.size(); i++ )
        {
            ...
            logger.debug( "At line 853 of DECServlet.java eBean: " + eBean.getPREFERRED_DEFINITION() + "**" + eBean.getLONG_NAME() + "**" + eBean.getCONCEPT_IDENTIFIER() );
        }
        //begin of GF30798
            ...
            if( !conName.equals( "" ) )
            {
                ...
                if( newBean == null )
                {
                    ...
                    if( !sDef.equals( "" ) )
                        sDef += "_"; // add definition
                    sDef += eCon.getPREFERRED_DEFINITION() + nvpValue;
                    logger.debug( "At line 883 of DECServlet.java sLongName, sDef: " + sLongName + "**" + sDef );
                }
                if( !sAbbName.equals( "" ) )
                    sAbbName += "_";
                if( conName.length() > 3 )
            ...
            if( !sPrimary.equals( "" ) )
            {
                ...
                if( newBean == null )
                {
                    ...
                    if( !sDef.equals( "" ) )
                        sDef += "_"; // add definition
                    sDef += eCon.getPREFERRED_DEFINITION() + nvpValue;
                    logger.debug( "At line 923 of DECServlet.java sLongName, sDef: " + sLongName + "**" + sDef );
                }
                if( !sAbbName.equals( "" ) )
                    sAbbName += "_";
                if( sPrimary.length() > 3 )
        ...
        if( newBean != null )
        {
            ...
            logger.debug( "At line 956 of DECServlet.java sLongName, sDef: " + sLongName + "**" + sDef );
        }
...
//	        session.setAttribute(Constants.DEC_EVS_LOOKUP_FLAG, false);
            ...
            String userSelectedVocab = /*CURATNTOOL-1046*/ StringUtil.cleanJavascriptAndHtml( ( String ) m_classReq.getParameter( "userSelectedVocab" ) );
...
//			}
            ...
            String sComp = /*CURATNTOOL-1046*/ StringUtil.cleanJavascriptAndHtml( ( String ) m_classReq.getParameter( "sCompBlocks" ) );
            if( sComp == null )
                sComp = "";
//			else
//				(new AltNamesDefsSessionHelper()).setCompType(sComp, session);		//JR1016
            ...
            if( sSelRow != null && !sSelRow.equals( "" ) )
            {
                ...
                String sNVP = /*CURATNTOOL-1046*/ StringUtil.cleanJavascriptAndHtml( ( String ) m_classReq.getParameter( "nvpConcept" ) );
...
//			session.removeAttribute("changed"+type+"DefsWarning");
     ...
     * @param ocType     String property type (primary or qualifier)
     * @return DEC_Bean
     * @throws Exception
     */
    @SuppressWarnings( "unchecked" )
    private DEC_Bean addOCConcepts(
            String nameAction,
     ...
     * @param decBean    selected DEC_Bean
     * @param eBean      selected EVS_Bean
     * @param propType   String property type (primary or qualifier)
     * @return DEC_Bean
     * @throws Exception
     */
    @SuppressWarnings( "unchecked" )
    private DEC_Bean addPropConcepts(
            String nameAction,
    ...
    private void doInsertDEC() throws Exception
    {
        ...
        if( sDECEditAction == null )
            sDECEditAction = "";
        String sAction = StringUtil.cleanJavascriptAndHtml( ( String ) m_classReq.getParameter( "ValidateDECPageAction" ) );
        if( sAction == null )
